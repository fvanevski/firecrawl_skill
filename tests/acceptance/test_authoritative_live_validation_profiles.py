...
class _Inspector:
    ...
    def probe_qdrant_alias(self):
        return {"alias": "research_chunks_active", "collection": "research_chunks_test", "dimension": 1024, "compatible": True}

    def wait_for_worker(self, run_id, **_kwargs):
        return {"external_run_id": run_id, "checks": {"authoritative_records": True}, "pass": True}
...
def test_contract_conformance_does_not_imply_capability_success(tmp_path: Path):
    ...
    campaign = validation.Campaign(_args(tmp_path), inspector=_Inspector(), runner=runner, real_cli="/usr/bin/firecrawl", work_root=tmp_path / "work")
    try:
        case = campaign.run(
            "typed_partial", ["fresearch"], json_output=True,
            expected_returncodes=(0, 1, 75), required_capability=True,
            capability_evaluator=lambda value, rc: bool(rc == 0 and value and value.get("disposition") == "terminal_completed" and value.get("objective_satisfied") is True),
        )
...
def test_retired_smart_matrix_requires_zero_provider_activity(tmp_path: Path):
    ...
    try:
        ...
        retired = [case for case in campaign.cases if case["name"].startswith("retired_smart_option_")]
...
def test_cleanup_terminalizes_only_validator_owned_nonterminal_runs(tmp_path: Path):
    ...
    inspector.states = {preexisting: "acquiring", owned: "acquiring", terminal: "completed"}
    calls: list[list[str]] = []
    def runner(command, **_kwargs):
    ...
    campaign = validation.Campaign(_args(tmp_path), inspector=inspector, runner=runner, real_cli="/usr/bin/firecrawl", work_root=tmp_path / "work")
    ...
    try:
        ...
        assert [command[2] for command in calls if len(command) >= 3 and command[1] == "cancel"] == [owned]
    finally:
        campaign.close()


def test_failure_path_dispatch_uses_current_matrix_without_legacy_smart_run(tmp_path: Path):
    validation = validation_module()
...
def test_destructive_profile_faults_only_positive_disposable_identity(tmp_path: Path):
    ...
    def runner(command, **_kwargs):
        ...
        if command[0] == str(SCRIPTS / "disposable-test-services"):
            ...
            if action == "env" and pg_port == 55432:
                return subprocess.CompletedProcess(command, 1, stdout="", stderr="reserved")
            if action == "up":
                namespace = command[command.index("--namespace") + 1]
                db = namespace.replace("-", "_") + "_test"
                qurl = f"http://127.0.0.1:{qdrant_port}"
                payload = {
                    "schema_version": "firecrawl-disposable-services-v1", "namespace": namespace,
                    "postgres": {"port": pg_port, "database": db}, "qdrant": {"port": qdrant_port},
                    "environment": {
                        "RESEARCH_STORE_TEST_DATABASE_URL": f"postgresql://postgres:postgres@127.0.0.1:{pg_port}/{db}",
        ...
        if command[:2] == [str(SCRIPTS / "research-db"), "ingest-ready"]:
            ingest_calls += 1
            return subprocess.CompletedProcess(command, 1 if ingest_calls == 2 else 0, stdout="", stderr="")
        raise AssertionError(command)
    args = _args(tmp_path, profile="destructive", max_operations=10, expected_head_sha=head, disposable_namespace="fc359", disposable_pg_port=55436, disposable_qdrant_port=55437)
    ...
    helper_commands = [command for command in commands if command and command[0] == str(SCRIPTS / "disposable-test-services")]
    protected = [command for command in helper_commands if command[command.index("--pg-port") + 1] == "55432"]
    ...
    assert all(command[command.index("--pg-port") + 1] == "55436" for command in destructive_starts)
    assert all(command[command.index("--qdrant-port") + 1] == "55437" for command in destructive_starts)


def test_successful_capability_is_distinct_positive_evidence(tmp_path: Path):
    validation = validation_module()
    payload = {"schema_version": "authoritative-fscrape-v1", "status": "complete"}
    def runner(command, **_kwargs):
        return subprocess.CompletedProcess(command, 0, stdout=json.dumps(payload), stderr="")
    campaign = validation.Campaign(_args(tmp_path), inspector=_Inspector(), runner=runner, real_cli="/usr/bin/firecrawl", work_root=tmp_path / "work")
    try:
        case = campaign.run("successful_capability", ["fscrape"], json_output=True, expected_schema="authoritative-fscrape-v1", required_capability=True, capability_evaluator=lambda value, rc: bool(rc == 0 and value and value.get("status") == "complete"))
        assert case["contract_result"] == "PASS"
...
def test_wrongly_typed_failure_is_contract_failure(tmp_path: Path):
    validation = validation_module()
    def runner(command, **_kwargs):
        return subprocess.CompletedProcess(command, 5, stdout=json.dumps({"schema_version": "wrong-v1", "failure_stage": "extraction"}), stderr="")
    campaign = validation.Campaign(_args(tmp_path), inspector=_Inspector(), runner=runner, real_cli="/usr/bin/firecrawl", work_root=tmp_path / "work")
    try:
        case = campaign.run("wrong_failure_contract", ["fscrape"], json_output=True, expected_returncodes=(5,), expected_schema="authoritative-fscrape-error-v1")
...
def test_accounting_separates_matrix_plumbing_and_not_run(tmp_path: Path):
    validation = validation_module()
    campaign = validation.Campaign(_args(tmp_path), inspector=_Inspector(), real_cli="/usr/bin/firecrawl", work_root=tmp_path / "work")
    try:
        campaign._record("matrix-pass", category="matrix", contract_result="PASS")
        campaign._record("matrix-not-run", category="matrix", contract_result="NOT_EVALUATED", required_contract=False)
...
def test_no_baseline_never_claims_unrelated_runs(tmp_path: Path):
    ...
    campaign = validation.Campaign(_args(tmp_path), inspector=inspector, real_cli="/usr/bin/firecrawl", work_root=tmp_path / "work")
    try:
...
def test_objective_bound_discovery_does_not_claim_concurrent_unrelated_run(tmp_path: Path):
    validation = validation_module()
    inspector = _Inspector()
    unrelated = "fr_" + "e" * 32
    owned = "fr_" + "f" * 32
...
def test_cleanup_failure_is_machine_readable_failure(tmp_path: Path):
    ...
    def runner(command, **_kwargs):
        return subprocess.CompletedProcess(command, 1, stdout="", stderr="cancel failed")
    campaign = validation.Campaign(_args(tmp_path), inspector=inspector, runner=runner, real_cli="/usr/bin/firecrawl", work_root=tmp_path / "work")
    campaign.run_baseline_captured = True
    campaign._track_run("owned", owned, "owned")
    try:
...
def test_keep_runs_is_reported_and_cannot_be_clean_cleanup(tmp_path: Path):
    ...
    inspector.run_ids = {owned}
    inspector.states[owned] = "created"
    campaign = validation.Campaign(_args(tmp_path, keep_runs=True), inspector=inspector, real_cli="/usr/bin/firecrawl", work_root=tmp_path / "work")
    campaign.run_baseline_captured = True
    campaign._track_run("owned", owned, "owned")
    try:
        evidence = campaign.cleanup_runs()
...
def test_malformed_disposable_receipt_still_tears_down_started_namespace(tmp_path: Path):
    validation = validation_module()
    head = "a" * 40
    commands: list[list[str]] = []
    def runner(command, **_kwargs):
        ...
        if command[0] == str(SCRIPTS / "disposable-test-services"):
            ...
            if action == "env" and pg_port == 55432:
                return subprocess.CompletedProcess(command, 1, stdout="", stderr="reserved")
            if action == "up":
                return subprocess.CompletedProcess(command, 0, stdout=json.dumps({"schema_version": "wrong-v1"}), stderr="")
            if action == "down":
                return subprocess.CompletedProcess(command, 0, stdout="", stderr="")
        raise AssertionError(command)
    args = _args(tmp_path, profile="destructive", max_operations=10, expected_head_sha=head, disposable_namespace="fc359bad")
    campaign = validation.DisposableDestructiveCampaign(args, runner=runner)
    ...
    def runner(command, **_kwargs):
        ...
        if command[0] == str(SCRIPTS / "disposable-test-services"):
            ...
            if action == "up":
                ...
                db = namespace.replace("-", "_") + "_test"
                qurl = f"http://127.0.0.1:{qdrant_port}"
                payload = {"schema_version": "firecrawl-disposable-services-v1", "namespace": namespace, "postgres": {"port": pg_port, "database": db}, "qdrant": {"port": qdrant_port}, "environment": {"RESEARCH_STORE_TEST_DATABASE_URL": f"postgresql://postgres:postgres@127.0.0.1:{pg_port}/{db}", "RESEARCH_STORE_TEST_ALLOW_RESET": db, "QDRANT_URL": qurl, "RESEARCH_STORE_TEST_QDRANT_URL": qurl, "RESEARCH_STORE_TEST_QDRANT_ALLOW_RESET": qurl}}
                return subprocess.CompletedProcess(command, 0, stdout=json.dumps(payload), stderr="")
            if action == "down":
                return subprocess.CompletedProcess(command, 1, stdout="", stderr="teardown failed")
        if command[:2] == [str(SCRIPTS / "research-db"), "migrate"]:
            return subprocess.CompletedProcess(command, 0, stdout="", stderr="")
        if command[:2] == [str(SCRIPTS / "research-db"), "ingest-ready"]:
            ingest_calls += 1
            return subprocess.CompletedProcess(command, 1 if ingest_calls == 2 else 0, stdout="", stderr="")
        raise AssertionError(command)
    args = _args(tmp_path, profile="destructive", max_operations=10, expected_head_sha=head, disposable_namespace="fc359down")
    campaign = validation.DisposableDestructiveCampaign(args, runner=runner)
    campaign._inject_schema_fault = mock.Mock()
    assert campaign.execute() == 1
    assert campaign._service_started is True
    assert any(case["name"] in {"disposable_fault_teardown", "disposable_final_teardown"} and case["contract_result"] == "FAIL" for case in campaign.cases)
