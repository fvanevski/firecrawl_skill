...
def test_legacy_service_is_identity_preserving_facade() -> None:
    legacy = importlib.import_module("firecrawl_skill.research_store.service")
    claims = importlib.import_module("firecrawl_skill.research_store.assessment.claims")
    audit = importlib.import_module("firecrawl_skill.research_store.assessment.audit")
    ...
    tree = ast.parse(
        (ROOT / "src" / "firecrawl_skill" / "research_store" / "service.py").read_text(
            encoding="utf-8"
        )
    )
    class_names = {
...
def test_baseline_tracked_assessment_modules_use_explicit_bridges() -> None:
    ...
    root_evidence = importlib.import_module("firecrawl_skill.research_store.evidence")
    root_binding = importlib.import_module(
        "firecrawl_skill.research_store.claim_binding_service"
    )
    root_validation = importlib.import_module(
...
def test_reporting_ownership_and_baseline_bridges_are_explicit() -> None:
    ...
    root_construction = importlib.import_module(
...
def test_completion_and_persistence_authority_stay_outside_topology_facades() -> None:
    completion_source = (
        ROOT / "src" / "firecrawl_skill" / "research_store" / "completion_provenance.py"
    ).read_text(encoding="utf-8")
    service_source = (
        ROOT / "src" / "firecrawl_skill" / "research_store" / "service.py"
    ).read_text(encoding="utf-8")
