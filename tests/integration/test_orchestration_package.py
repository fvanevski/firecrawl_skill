...
A. Import boundary: importing research_store does NOT mutate
   research_store.orchestrator symbol identity or recreate application builders.
...
class TestImportBoundary:
    ...
    def test_import_research_store_does_not_mutate_orchestrator_identity(self):
        """Verify package import preserves run identity and builder absence."""
        import subprocess
        ...
        script = (
            ...
            "assert not hasattr(orch.ResearchOrchestrator, 'build'), 'build unexpectedly present'\n"
            ...
            "assert not hasattr(after.ResearchOrchestrator, 'build'), 'build recreated'\n"
            "print('OK')\n"
        )
...
class TestCommandDelegation:
    """Facades hand an actual ``RunResearchCommand`` to the use cases."""
...
class TestNoRawSQLInOrchestration:
    """AC-I: No raw SQL / cursor usage in orchestration application package."""
...
class TestNoRawSQLInSmartOrchestrator:
    ...
    def test_no_execute_sql_calls(self, smart_file: Path):
        tree = ast.parse(smart_file.read_text())
        sql_keywords = ("SELECT ", "INSERT ", "UPDATE ", "DELETE ")
        for node in ast.walk(tree):
            if not (
                isinstance(node, ast.Call)
...
class TestResumeReaderIntegration:
    """AC-L: PostgresResumeStateReader live round-trip over canonical repos."""
    ...
    def test_reader_composes_canonical_repo_read(self):
        ...
        seed: list[tuple[LiteralString, tuple[str, ...]]] = [
            (
                "INSERT INTO research_runs (id, objective, query_plan, skill_version, llm_model, state, execution_mode) VALUES (%s, 'resume-reader', '{}', '1.0', 'm', 'created', 'agent_led')",
                (str(run_id),),
            ),
            (
                "INSERT INTO sources (id, canonical_url, source_type, registered_domain) VALUES (%s, %s, 'web', 'example.com')",
                (str(source_id), url),
            ),
            (
            ...
            (
                "INSERT INTO chunks (id, document_id, ordinal, text, content_sha256, token_count, chunker_name, chunker_version, tokenizer_name) VALUES (%s, %s, 0, 'a', %s, 1, 'structural-v1', '1.0', 'cl100k_base')",
                (str(chunk_a), str(document_id), sha("a")),
            ),
            (
                "INSERT INTO chunks (id, document_id, ordinal, text, content_sha256, token_count, chunker_name, chunker_version, tokenizer_name) VALUES (%s, %s, 1, 'b', %s, 1, 'structural-v1', '1.0', 'cl100k_base')",
...
class TestPublicExports:
    """Verify orchestration package exposes expected public API."""
