...
class TestExtractEndpointUsage:
    ...
    def test_usage_in_response_metadata(self):
        from firecrawl_skill.research_store.token_accounting import extract_endpoint_usage
...
class TestTelemetryService:
    ...
    def test_strict_pass_false_when_unavailable_tokens(self):
        ...
        _summary = PerformanceTelemetrySummary(
            run_id=str(uuid4()), token_source="unavailable", strict_pass=True
        )
...
class TestStrictModeRejection:
    ...
    def test_strict_mode_rejects_no_cpu_samples(self):
        ...
        summary = PerformanceTelemetrySummary(
            run_id=str(uuid4()), cpu_samples=0, strict_pass=True
        )
    ...
    def test_strict_mode_produces_null_metrics_when_telemetry_tables_absent(self):
        ...
        from firecrawl_skill.research_store.release.benchmark import MetricEngine, ReleaseBenchmarkConfig
        ...
        engine.config = ReleaseBenchmarkConfig(
            database_url="postgresql://fake", blob_root=Path("/tmp"), strict=True
        )
    ...
    def test_strict_mode_blocks_legacy_fallbacks(self):
        ...
        mock_telemetry_cursor.fetchone.return_value = (
            None, None, 0, 0, 0, 0, 0.0, 0, 0.0, None, 0, None, 0
        )
    ...
    def test_strict_cache_metric_status_is_unavailable(self):
        ...
        from firecrawl_skill.research_store.release.benchmark import MetricEngine, MetricStatus, ReleaseBenchmarkConfig
        ...
        assert cache_metric.source.run_id is not None and cache_metric.source.run_id != ""
    ...
    def test_non_strict_cache_metric_status_is_measured_with_lookups(self):
        ...
        mock_telemetry_cursor.__exit__ = mock.Mock(return_value=False)
        mock_telemetry_cursor.fetchone.return_value = (
            1000, "endpoint", 10, 3, 7, 1, 50.0, 48, 2.5, 45.0, 5, 1024.0, 3
        )
        mock_conn.execute.return_value = mock_telemetry_cursor
        mock_cursor = mock.Mock()
        mock_cursor.__enter__ = mock.Mock(return_value=mock_cursor)
        ...
        engine._connection = mock_conn
        engine.config = ReleaseBenchmarkConfig(
            database_url="postgresql://fake", blob_root=Path("/tmp"), strict=False
        )
        _, metrics = engine.extract_performance_metrics(uuid4(), time.monotonic())
        cache_metric = next((m for m in metrics if m.name == "cache_hit_rate"), None)
        assert cache_metric is not None
        assert cache_metric.status == MetricStatus.MEASURED
        assert cache_metric.value == 0.3
        assert cache_metric.source.table == "run_cache_events"
    ...
    def test_strict_cpu_rejects_legacy_when_tables_absent(self):
        ...
        performance, metrics = engine.extract_performance_metrics(uuid4(), time.monotonic())
        assert performance.cpu_percent is None
...
class TestRegression:
    ...
    def test_release_benchmark_no_estimate_in_telemetry_path(self):
        import firecrawl_skill.research_store.release.benchmark as benchmark_module

        with open(benchmark_module.__file__) as f:
            source = f.read()
...
class TestBenchmarkTelemetryWiring:
    def test_populate_endpoint_usage_from_semantic_calls(self):
        from firecrawl_skill.research_store.release.benchmark import ReleaseBenchmarkRunner
        ...
        record = mock_telemetry_svc.record_endpoint_usage.call_args[0][0]
...
def test_resource_sampler_backfills_complete_window_on_initial_gpu_sample(monkeypatch):
    ...
    fake_nvml = SimpleNamespace(
        ...
        nvmlDeviceGetMemoryInfo=lambda _handle: SimpleNamespace(used=1024 * 1024 * 2048),
    )
    ...
    sampler = resource_sampler.ResourceSampler(interval_seconds=0.01, max_samples=2)
    sampler.begin_window()
    ...
    assert {sample.window_start for sample in gpu_samples} == {gpu_samples[0].window_start}
...
def test_periodic_resource_window_collects_interior_samples():
    ...
    fake_nvml = SimpleNamespace(
        ...
        nvmlDeviceGetMemoryInfo=mock.Mock(return_value=SimpleNamespace(used=512 * 1024 * 1024)),
    )
    with (
        mock.patch.object(module, "_HAS_PSUTIL", True),
        mock.patch.object(module, "psutil", fake_psutil, create=True),
    ...
    assert [sample.sample_number for sample in cpu_samples] == list(range(len(cpu_samples)))
    assert [sample.sample_number for sample in gpu_samples] == list(range(len(gpu_samples)))
