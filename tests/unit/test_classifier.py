...
def test_ecommerce_keyword_patterns():
    ...
    category, is_match = classify_target(
        "https://example.com/somepage",
        "Premium Widget",
        "Price: $49.99, In Stock today!",
    )
    assert category == "ecommerce"
    assert is_match is True
    ...
    category, is_match = classify_target(
        ...
        "Underpants - Buy Now",
        "Add to cart for 15% off with sku-1002",
    )
    assert category == "ecommerce"
    assert is_match is True
...
def test_forum_keyword_patterns():
    ...
    category, is_match = classify_target(
        ...
        "Posted by tech_guy 5 hours ago. 12 replies.",
    )
    assert category == "forum"
    assert is_match is True
    ...
    category, is_match = classify_target(
        "https://example.com/page",
        "Mesa compilation updates",
        "This is an ongoing discussion thread with 42 comments.",
    )
    assert category == "forum"
    assert is_match is True
...
def test_news_article_patterns():
    ...
    category, is_match = classify_target(
        "https://example.com/page",
        "Breaking Update",
        "Published on June 24, 2026. Reported by Reuters news desk.",
    )
    assert category == "news_article"
    assert is_match is True
...
def test_media_release_patterns():
    ...
    category, is_match = classify_target(
        "https://example.com/page",
        "Film Premiere Today",
        "Watch trailer and find showtimes for the new theatrical release starring famous actors.",
    )
    assert category == "media_release"
    assert is_match is True
...
def test_academic_debate_patterns():
    ...
    category, is_match = classify_target(
        ...
        "We examine Premise 1, Premise 2, and whether the conclusion follows using a deductive syllogism.",
    )
    assert category == "academic_debate"
    assert is_match is True
