...
def test_checked_in_schema_matches_generated_schema(version):
    # Skip non-current (legacy) versions — their schema const differs from
    # the model's SCHEMA_VERSION because they are generated from the current
    # model class.  Legacy schemas are checked in manually with the correct
    # const value and are not regenerated by write_schemas().
...
def test_v1_compatibility_policy_is_explicit_and_rejects_unknown_versions():
    for version, policy in COMPATIBILITY_POLICY.items():
        current = policy.get("current", True)
        readable = policy.get("readable_versions", (version,))
        write_ver = policy.get("write_version", version)
        # Legacy versions have structured readable_versions/write_version
        if not current:
            assert isinstance(readable, tuple)
            assert isinstance(write_ver, str)
            continue
        assert policy == {
            "current": True,
