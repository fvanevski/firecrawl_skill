...
The changed-scope CI gate checks the actual ACMR non-test Python paths from the exact PR base/head (excluding the historical test corpus, `test_*.py`), in addition to the full-project Pyrefly gate.
...
2. repository-pinned Pyrefly on the explicit changed non-test Python set (the historical test corpus, including `tests/contract/test_release_package_boundary.py`, is excluded from the changed-scope set, matching the CI gate);
