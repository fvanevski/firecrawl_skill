...
A recoverable command deadline or batch bound emits a structured `index-drain-result-v1` with status `resumable`, exit status `75`, the last worker result, and the last exact census. Cancellation emits status `cancelled` with exit status `130`. The helper performs no lifecycle transition, so the run remains resumable and nonterminal. Unscoped invocation remains compatible for projection maintenance, but its queue-empty result is not run-completion evidence.
