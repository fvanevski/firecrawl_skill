...
1. `pre_extraction`: candidate composition and projected extraction attempts are checked before constructing or invoking direct-scrape transport. In the canonical planned path, hard violations are enforced at this boundary; soft violations remain soft and retain the existing explicit-override contract rather than being silently promoted to non-overridable failures.
