...
- Payload bytes are durably installed before PostgreSQL commits metadata that references their digest.
- Stable authoritative IDs are returned only after the corresponding PostgreSQL commit.
...
`research-db worker --once` processes at most one bounded batch. It is not sufficient by itself to prove that a run or rebuild has completed. The drain helper repeats bounded batches until one reports `claimed=0` and fails closed on worker errors, invalid output, failed jobs, lease loss, or an exceeded bound.

Do not issue another acquisition on the same run or call `frun finish` while run-scoped indexing is unfinished. To add `fscrape` to the same run, drain and verify the prior jobs, run `fscrape`, then drain again.
...
The canonical transaction, acquisition, completion, and Qdrant-recovery recipes are in `authoritative-workflows.md`.
...
- Qdrant can be rebuilt after upgrade only after all durable jobs are drained and reconciliation succeeds.
...
4. rebuild Qdrant, drain all durable jobs, reconcile, and activate, or reactivate a verified compatible prior index;
5. recreate Valkey;
...
- executable lifecycle and multi-batch drain tests;
...
- blob-write failure before metadata suppression;
- metadata rollback after blob installation producing at most an unreferenced orphan;
- authoritative IDs returned only after commit;
- idempotent search/scrape replay;
- blob integrity;
- worker restart and lease recovery;
- Qdrant rebuild and rollback after complete drain and reconciliation;
