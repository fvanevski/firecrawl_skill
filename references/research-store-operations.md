...
On the configured host, sourcing `scripts/research-env` derives `RESEARCH_STORE_TEST_DATABASE_URL` for the separate `research_assets_test_codex` database and sets the exact reset acknowledgement. Override `RESEARCH_STORE_TEST_DATABASE` before sourcing to select another disposable database; explicit test DSN and acknowledgement values remain authoritative. The adapter only constructs the DSN—it does not create or migrate the database.
