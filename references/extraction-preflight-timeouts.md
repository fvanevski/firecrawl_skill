...
the audited run. Issue #305 retains those bounds while adding one dedicated,
conservative retry budget for first-byte timeouts.
...
3. Extraction creates an extraction-attempt record and performs bounded
   `firecrawl scrape` provider sub-attempts for one candidate.
...
| `FIRECRAWL_EXTRACTION_FIRST_BYTE_TIMEOUT_RETRIES` | `1` | Additional provider sub-attempts allowed only after `first_byte_timeout`; independent of generic transient retries and still bounded by the overall candidate deadline. |
...
A first-byte retry starts only after that reap has completed. The overall
candidate deadline remains authoritative across all sub-attempts, so increasing
the retry count does not authorize unbounded provider work. Timeout outcomes
remain classified as `timeout`, never `empty_content`.
...
| `first_byte_timeout` | `FIRECRAWL_EXTRACTION_FIRST_BYTE_TIMEOUT_RETRIES` (default 1), independently bounded by overall candidate deadline | Reap timed-out child, retry; after exhaustion retain stable first-byte timeout reason/count | `timeout` |
| `provider_operation_timeout` | none | Terminate/reap provider work; do not convert to generic retryable transport failure | `timeout` |
| `overall_candidate_timeout` | none | Terminate/reap provider work | `timeout` |
...
A timeout→retry→success sequence still produces one candidate-level result and
one durable candidate outcome. Bounded transport telemetry records compact
`provider_sub_attempts` entries (sub-attempt ordinal, classification/reason,
exit code, first-byte/operation timing, timeout reason, and cancellation state)
so operators can explain the internal retry without double-counting durable
candidate attempts.
...
No Alembic migration is required. Issue #216 and the issue #305 first-byte retry
reuse existing `extraction_attempts` columns and the existing
`extraction_failure_class` enum. Timing/classification and bounded sub-attempt
telemetry are carried in transport/preflight metadata; the public
`SearchAdapterResult` domain contract is not expanded. Persisted audit output
uses existing PostgreSQL fields. Existing search-response and immutable content
hashing contracts are unchanged.

Rollback is therefore code-only: reverting the bounded provider/stage routing
restores the previous behavior without a database downgrade. Historical rows
are not backfilled or reclassified.
...
| First-byte/provider/overall deadlines were post-hoc or absent and provider work could be orphaned. | `BoundedSubprocessRunner` enforces live deadlines, starts a separate process session, terminates the process group, escalates to `SIGKILL` when required, and waits for reaping. Issue #305 permits a new first-byte provider sub-attempt only after that reap. | Real child-process first-byte/provider-operation timeout tests plus issue #305 retry→success/exhaustion tests. |
| Timeout provenance was conflated with empty content and timing was lost. | Timeout has its own classification/reason code/failure stage and elapsed timing is propagated in preflight metadata. Exhausted first-byte retry preserves `first_byte_timeout` and bounded attempt count. | Timeout-classification tests and PostgreSQL audit readback. |
...
| New tests did not exercise the responsible production seam and were absent from CI. | The issue suite imports canonical package routing, executes `BoundedExtractionStage`, uses real child processes for cancellation, and performs PostgreSQL readback. | Existing extraction-preflight workflow plus issue #305 focused/broader validation. |
| Superseded first-pass API and broad test-format churn remained in the diff. | Production changes stay at the bounded provider/stage seam; no persistence authority or public provider domain model is redesigned for retry. | Final PR diff inspection. |
...
`tests/integration/test_issue_216_extraction_preflight.py` plus issue #305 focused
regressions cover:
...
- generic transient retry exhaustion/success;
- distinct timeout classes;
- first-byte process cancellation/reaping;
- first-byte timeout→success and exhausted first-byte retry behavior;
- provider-operation timeout remaining non-retryable by the first-byte budget;
...
- documentation/CI synchronization for the bounded extraction contract.

Service-backed acceptance must use the repository disposable-service boundary;
no result against a persistent PostgreSQL/Qdrant service is authoritative test
