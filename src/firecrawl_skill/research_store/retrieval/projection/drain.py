...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run bounded index-worker batches against an optional sealed research-run census."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the research-db operator command.",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    research_db = Path(args.research_db) if args.research_db else Path("research-db")
    stop = Event()
