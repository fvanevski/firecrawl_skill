...
def replay_completed_checkpoint(
    service: IndexCheckpointService, run_id: UUID
) -> IndexFinalization:
    ...
    with service.uow_factory() as uow, uow.connection.cursor() as cursor:
        ...
        if state != "coverage_review":
            raise IndexCheckpointError(
                f"completed checkpoint replay requires unchanged coverage_review state; got {state}"
            )
        checkpoint = service._latest(
            cursor, run_id, statuses=("completed",), for_update=True
        )
        ...
        if (
            ...
            raise IndexCheckpointError(
                "completed checkpoint replay failed: authoritative census no longer proves exact completion"
            )
        ...
        if row is None:
            raise IndexCheckpointError(
                "completed checkpoint replay failed: matching lifecycle transition is missing"
            )
        validation_result = row[5] or {}
        if not isinstance(validation_result, dict):
            raise IndexCheckpointError(
                "completed checkpoint replay failed: transition validation payload is malformed"
            )
        if validation_result.get("completion") != expected_completion:
            raise IndexCheckpointError(
                "completed checkpoint replay failed: transition payload does not match the persisted checkpoint"
            )
        transition = {
            "transition_id": row[0],
            "event_id": row[1],
