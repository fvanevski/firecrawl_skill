"""Core lifecycle for durable PostgreSQL indexing checkpoints."""
...
class _IndexCheckpointCoreMixin:
    ...
    def __init__(self, uow_factory: Any, *, max_attempts: int = 5):
        if max_attempts <= 0:
    ...
    def active_fingerprint(self, run_id: UUID) -> str:
        with self.uow_factory() as uow, uow.connection.cursor() as cursor:
            cursor.execute(
                """SELECT definition.fingerprint FROM index_definitions definition
        ...
        if len(fingerprints) != 1:
            raise IndexCheckpointError(
                f"expected one configured index fingerprint for run {run_id}, found {len(fingerprints)}"
            )
        ...
        *,
        ...
        with self.uow_factory() as uow, uow.connection.cursor() as cursor:
            ...
            if int(current_revision) != lifecycle_revision:
                raise IndexCheckpointStaleError(
                    f"checkpoint revision is stale: expected {lifecycle_revision}, current {current_revision}"
                )
            ...
            cursor.execute(
                """INSERT INTO indexing_checkpoints(
                       ...
                       expected_membership_sha256,expected_count,deadline_at,idempotency_key)
