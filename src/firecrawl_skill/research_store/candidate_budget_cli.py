"""Operator CLI implementation for candidate-budget checks and soft overrides.

Operator execution is routed through ``scripts/candidate-budget`` so
``scripts/research-env`` establishes exact-checkout runtime provenance before
...
from .composition import build_run_service
