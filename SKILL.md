...
The narrow deterministic grammar accepts explicit ISO ranges, `past N days`, and bounded month-name ranges such as `August 18-23, 2026` or `from August 18 to August 23, 2026`. It is **not** a normal autonomous fallback. It is used only by explicit `deterministic_debug`/degraded fixtures and authoring helpers. Normal `autonomous_local` execution fails closed after semantic model, schema, post-validation, ambiguity, or unsupported-intent failure and points the operator to an explicit `--research-spec`; it never silently switches to regex parsing.
