...
def probe_local(base_url, api_key=""):
    ...
    try:
        with urlopen(
            Request(base_url + "/models", headers=headers), timeout=5
        ) as response:
    ...
    *,
    ...
    for attempt_number in range(1, max_attempts + 1):
        ...
        try:
            ...
            content, envelope = (
                _gemini_content(raw) if provider == "gemini" else _json_content(raw)
            )
            try:
                ...
                attempts.append(
                    {
                        ...
                        "content_excerpt": _redact(content)[:MAX_RAW_EXCERPT],
                        "schema_valid": False,
                        "validation_errors": [_redact(last_error)],
                        **envelope,
                    }
                )
                continue
            ...
            if valid:
                ...
                artifact_ids = (
                    semantic_persistence.finish_model_call(
                        context,
                        call_id,
                        status="complete",
