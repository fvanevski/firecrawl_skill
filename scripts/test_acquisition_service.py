...
def test_acquisition_service_normal_flow(tmp_path, prepared_database):
    ...
    run_svc, status = _prepared_run(config, "test acquisition service", ext_id)
    run_id = status.id
    mock_adapter = MockSuccessSearchAdapter()
    acq_svc = build_acquisition_service(config, search_adapter=mock_adapter)
    res = acq_svc.execute_search(run_id, "machine learning tutorials")
...
def test_acquisition_service_idempotent_retry(tmp_path, prepared_database):
    """Same idempotency key must not create duplicate candidates/responses."""
    migrate(TEST_DSN)
    config = replace(
        StoreConfig.from_env(), database_url=TEST_DSN, blob_root=tmp_path / "blobs"
    )
    ext_id = f"run-acq-retry-{uuid4()}"
    run_svc, status = _prepared_run(config, "test search retries", ext_id)
    run_id = status.id
    mock_adapter = MockSuccessSearchAdapter()
    acq_svc = build_acquisition_service(config, search_adapter=mock_adapter)
...
def test_acquisition_service_conflicting_retry_fails_before_provider(
    ...
    run_svc, status = _prepared_run(config, "test search idempotency conflict", ext_id)
    run_id = status.id
    ...
    with pytest.raises(AcquisitionIdempotencyConflictError, match="another request"):
...
def test_acquisition_service_transport_error_persistence(tmp_path, prepared_database):
    ...
    run_svc, status = _prepared_run(config, "test transport error recording", ext_id)
    run_id = status.id
    mock_adapter = MockTransportErrorSearchAdapter()
    acq_svc = build_acquisition_service(config, search_adapter=mock_adapter)
    res = acq_svc.execute_search(run_id, "query causing transport failure")
    assert res.postgres_committed is True
...
def test_acquisition_service_crash_reconciliation(tmp_path, prepared_database):
    """Reconciling pending searches materializes candidates for stored responses."""
    migrate(TEST_DSN)
    config = replace(
        StoreConfig.from_env(), database_url=TEST_DSN, blob_root=tmp_path / "blobs"
    )
    ...
    payload = json.dumps(
        {
            ...
            "data": [
                {
                    "url": "https://reconcile.example.com/doc1",
                    "title": "Reconciled Doc",
                }
            ],
        }
    )
