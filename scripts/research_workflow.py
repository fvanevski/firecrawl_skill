        ...
        "queries": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "intended_source_class": {"type": "string"},
                    "expected_organizations": {
                        "type": "array",
                        "items": {"type": "string"},
                    },
                    "freshness_requirement": {"type": "string"},
                    "domain_neutral": {"type": "boolean"},
        ...
        "decisions": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "relevance": {
                        "type": "string",
                        "enum": ["high", "medium", "low", "unrelated", "uncertain"],
                    },
                    "source_suitability": {
                        "type": "string",
                        "enum": [
...
def plan_queries(
    ...
    *,
    ...
    if queries and not any(item.get("domain_neutral") for item in queries):
        queries[0]["domain_neutral"] = True
...
def candidate_cards(candidates):
    ...
    for index, item in enumerate(candidates):
        candidate_id = (
            ...
            or "triage_"
            + hashlib.sha256(f"{index}\0{item.get('url', '')}".encode()).hexdigest()[
                :20
            ]
        )
        ...
        cards.append(
            {
                ...
                "snippet": (item.get("snippet") or item.get("description") or "")[:700],
                "rank": item.get("rank"),
    ...
    *,
    ...
    for card in cards:
        if current and (
            ...
            or estimate_tokens(base + json.dumps(current + [card])) > target_tokens
        ):
    ...
    for batch_number, chunk in enumerate(chunks, 1):
        ...
        if result.value:
            ...
            decisions.extend(
                item
                for item in result.value.get("decisions", [])
                if item.get("candidate_id") in known
            )
    by_id = {item["candidate_id"]: item for item in decisions}
    ranked = []
    for item in triage_candidates_set:
        decision = by_id.get(
            item.get("triage_candidate_id"),
            {
                "relevance": "uncertain",
    ...
    ranked.sort(
        key=lambda item: (
            -item.get("triage", {}).get("priority", 0),
            item.get("rank", 9999),
            item.get("url", ""),
        )
    )
    return ranked, {
        ...
        "coverage": len(decisions) / len(triage_candidates_set)
        if triage_candidates_set
        else 1,
    }
...
def evidence_packet(
    ...
    return {
        ...
        "limitations": [
            "claim and excerpt bindings are completed when the research run source manifest is finalized"
        ],
    }
