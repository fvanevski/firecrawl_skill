...
class TestStatusSerialization:
    ...
    def test_status_fields_serialized_in_result_json(self):
        ...
        with mock.patch("research_store.strict_benchmark._preflight_check", return_value=(True, [])):
            from research_store.strict_benchmark import main
            rc = main([
                "--campaign-dir", str(campaign_dir),
                "--database-url", database_url,
                "--dataset", str(Path(__file__).resolve().parent.parent / "tests/fixtures/benchmark/benchmark-v1.json"),
                "--objectives", "obj-001",
                "--tolerance", "0.15",
            ])
            assert rc == 1
        ...
        import json
        result_json_path = list(campaign_dir.glob("A/*/result.json"))[0]
        ...
        assert "quality_metrics" in run
        assert "performance_metrics" in run
        
        # Verify status is serialized
        for m in run["quality_metrics"]:
            assert "status" in m
        for m in run["performance_metrics"]:
            assert "status" in m
