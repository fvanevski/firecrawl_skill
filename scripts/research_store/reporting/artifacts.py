"""Persistence for report artifacts, claim manifests, and validation results."""
...
from .validation import ReportValidationResult, ReportValidator
...
class ReportArtifactService:
    """Persist report validation artifacts in authoritative PostgreSQL state."""
                ...
                "claim_manifest": [
                    {
                        ...
                        "cited_passage_ids": list(manifest.cited_passage_ids),
                        "binding_relationship": manifest.binding_relationship,
                        "issues": list(manifest.issues),
                    }
                    for manifest in validation_result.claim_manifest
                ],
    ...
    def get_report(self, run_id: UUID) -> dict[str, Any] | None:
        with self._uow_factory() as uow:
            try:
