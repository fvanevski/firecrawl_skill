...
The search operation is discovery-only. Candidate extraction is performed by
``scrape_url`` as a separate child process with first-byte, provider-operation,
and overall-candidate deadlines. This keeps a slow or unsuitable candidate from
holding the extraction stage for an unbounded provider job.
"""
...
class BoundedFirecrawlSearchAdapter:
    ...
    def scrape_url(
        ...
        *,
        ...
        """Run one bounded candidate scrape and return an auditable outcome."""
        requested_at = utcnow()
