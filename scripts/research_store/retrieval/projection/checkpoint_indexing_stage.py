...
class CheckpointIndexingStage:
    ...
    def __init__(self, run_service, config, corpus_service: Any | None = None) -> None:
    ...
    def execute(
        ...
        try:
            checkpoint = checkpoints.ensure(
                ...
                idempotency_key=(
                    f"orchestrator:index-checkpoint:{run_id}:r{run_revision}"
                ),
            )
        except Exception as exc:  # noqa: BLE001
            return StageResult.failed(
        ...
        if drain.status != "complete":
            return StageResult.failed(
                "indexing",
                f"indexing failed closed: {drain.reason}",
                details=details,
            )
