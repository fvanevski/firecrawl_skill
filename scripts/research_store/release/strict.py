...
# Preserve the legacy path anchor for fixture/provenance semantics after the
# implementation moves under research_store.release.
SCRIPTS = Path(__file__).resolve().parents[1] / "strict_benchmark.py"
...
from research_store.release.workflow import load_benchmark_dataset
            ...
            "runs": [
                {
                    ...
                    "quality": {
                        "candidate_recall": run.quality.candidate_recall if run.quality else None,
                        "source_quality_score": run.quality.source_quality_score if run.quality else None,
                        "coverage_completeness": run.quality.coverage_completeness if run.quality else None,
                        "unsupported_claim_rate": run.quality.unsupported_claim_rate if run.quality else None,
                        "citation_accuracy": run.quality.citation_accuracy if run.quality else None,
                        "report_quality_score": run.quality.report_quality_score if run.quality else None,
                    }
                    if run.quality
                    else None,
                    "quality_metrics": [
                    ...
                    "performance": {
                        "total_latency_ms": run.performance.total_latency_ms if run.performance else None,
                        "total_tokens": run.performance.total_tokens if run.performance else None,
                        "semantic_calls": run.performance.semantic_calls if run.performance else None,
                        "cache_hit_rate": run.performance.cache_hit_rate if run.performance else None,
                        "embedding_throughput": run.performance.embedding_throughput if run.performance else None,
                        "cpu_percent": run.performance.cpu_percent if run.performance else None,
                        "gpu_memory_mb": run.performance.gpu_memory_mb if run.performance else None,
                    }
                    if run.performance
                    else None,
                    "performance_metrics": [
                    ...
                    "errors": run.errors,
                    "integrity_checks": [
                        {"check": c.check_name, "passed": c.passed, "details": c.details}
...
def _build_manifest(
    ...
    return {
        ...
        "campaign_a": {
            ...
            "result_hash": _compute_file_hash(campaign_a_dir / "result.json") if campaign_a_dir else None,
            ...
            "recommendation": result_a.recommendation.outcome if result_a.recommendation else None,
        },
        "campaign_b": {
            "campaign_id": result_b.campaign_id,
            "result_hash": _compute_file_hash(campaign_b_dir / "result.json") if campaign_b_dir else None,
            ...
            "recommendation": result_b.recommendation.outcome if result_b.recommendation else None,
        },
        "reproducibility": {
            "all_within_tolerance": comparison.all_within_tolerance,
            "run_a_id": comparison.run_a_id,
            ...
            "relative_tolerance": comparison.relative_tolerance,
            "operational_ratio_limit": comparison.operational_ratio_limit,
            "operational_absolute_tolerances": dict(comparison.operational_absolute_tolerances),
...
def main(
    ...
    parser.add_argument(
        ...
        help="Directory to write campaign artifacts (default: /tmp/firecrawl_strict_campaign)",
    )
    parser.add_argument(
        "--dataset", type=Path, default=DEFAULT_DATASET, help="Path to the benchmark dataset JSON file"
    )
    parser.add_argument(
        "--database-url", type=str, default=os.environ.get("DATABASE_URL", ""), help="PostgreSQL connection string"
    )
    parser.add_argument(
        "--blob-root", type=Path, default=Path(os.environ.get("BLOB_ROOT", "/tmp/benchmark-blobs")), help="Path to the content-addressed blob store root"
    )
    parser.add_argument(
        "--qdrant-url", type=str, default=os.environ.get("QDRANT_URL", ""), help="Qdrant URL"
    )
    parser.add_argument(
        "--qdrant-api-key", type=str, default=os.environ.get("QDRANT_API_KEY", ""), help="Qdrant API key"
    )
    parser.add_argument(
        "--objectives", type=str, default=None, help="Comma-separated objective IDs to run (default: all)"
    )
    parser.add_argument(
        "--tolerance", type=float, default=0.15, help="Reproducibility tolerance (default: 0.15)"
    )
    parser.add_argument(
        "--manifest", type=Path, default=None, help="Path to write the final manifest (default: <campaign-dir>/manifest.json)"
    )
    parser.add_argument(
        "--recovery-report", type=Path, default=None, help="Recovery report path (default: <campaign-dir>/recovery-report.txt)"
    )
    parser.add_argument(
        "--dry-run", action="store_true", default=False, help="Validate configuration without executing campaigns"
    )
    ...
    if len(candidate_sha) != 40 or not re.fullmatch(r"[0-9a-f]{40}", candidate_sha):
        print(
            f"ERROR: --candidate-sha must be a full 40-character hex string; got {len(candidate_sha)} chars: {candidate_sha!r}",
            file=sys.stderr,
        )
        return 1
    ...
    if not args.database_url:
        print("ERROR: --database-url is required (or DATABASE_URL env var)", file=sys.stderr)
        return 1
    ...
    recovery_lines = [
        ...
        "Campaign A run IDs:",
        *[f"- {run.mode}/{run.objective_id}: {run.run_id or 'MISSING'}" for run in result_a.runs],
        "Campaign B run IDs:",
        *[f"- {run.mode}/{run.objective_id}: {run.run_id or 'MISSING'}" for run in result_b.runs],
    ]
    if result_a.recommendation:
        recovery_lines.append(f"Campaign A recommendation: {result_a.recommendation.outcome}")
    if result_b.recommendation:
        recovery_lines.append(f"Campaign B recommendation: {result_b.recommendation.outcome}")
    ...
    if not comparison.all_within_tolerance:
        print(
            "\nWARNING: Reproducibility comparison FAILED. Out-of-tolerance differences detected."
        )
        return 1
    return 0
