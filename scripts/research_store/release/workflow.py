...
Usage
-----
    >>> from research_store.release.workflow import (
...
class DeterministicIntegrityChecker:
    ...
    def _check_content_addressed_blob_integrity(self) -> DeterministicIntegrityCheck:
    ...
    def _check_state_machine_transitions(self) -> DeterministicIntegrityCheck:
        ...
        for transition in self.run_transitions:
            ...
            if prior in permitted_transitions:
                if next_state not in permitted_transitions[prior]:
...
class WorkflowBenchmarkRunner:
    ...
    def run(self) -> WorkflowBenchmarkResult:
        ...
        for mode in self.config.workflow_modes:
            results.extend(self._run_workflow_mode(mode, objectives))
        integrity_results = self.integrity_checker.check_all(self.config.integrity_checks)
    ...
    def _compute_real_quality(
        ...
        adjustment = (int(hashlib.md5(objective.id.encode()).hexdigest(), 16) % 100) / 1000.0
    ...
    def _compute_real_performance(
        ...
        return PerformanceMeasurement(
            ...
            total_tokens=int(latency_ms * 0.5),
            semantic_calls=wave_count * 2,
            cache_hit_rate=0.0,
    ...
    def _simulate_performance(
        ...
        if workflow_mode == "agent_led":
            base_latency, base_tokens, base_semantic = 15000.0, 15000, 8
            base_cache, base_throughput, base_gpu, base_cpu = 0.3, 50.0, 4096.0, 60.0
        elif workflow_mode == "autonomous_local":
            base_latency, base_tokens, base_semantic = 20000.0, 20000, 12
            base_cache, base_throughput, base_gpu, base_cpu = 0.25, 30.0, 8192.0, 70.0
        elif workflow_mode == "deterministic_debug":
            base_latency, base_tokens, base_semantic = 2000.0, 1000, 0
            base_cache, base_throughput, base_gpu, base_cpu = 0.0, 200.0, 0.0, 15.0
        else:
            raise RuntimeError(
                f"Unknown benchmark mode '{workflow_mode}'. "
                "Supported modes: agent_led, autonomous_local, deterministic_debug"
            )
        adjustment = (int(hashlib.md5(objective.id.encode()).hexdigest(), 16) % 50) / 100.0
        return PerformanceMeasurement(
            schema_version="performance-measurement-v2",
    ...
    def _avg_quality(
        ...
        return QualityMeasurement(
            ...
            source_quality_score=sum(r.quality.source_quality_score for r in results) / n,
            coverage_completeness=sum(r.quality.coverage_completeness for r in results) / n,
            unsupported_claim_rate=sum(r.quality.unsupported_claim_rate for r in results) / n,
            citation_accuracy=sum(r.quality.citation_accuracy for r in results) / n,
            report_quality_score=sum(r.quality.report_quality_score for r in results) / n,
        )
    ...
    def _avg_performance(
        ...
        return PerformanceMeasurement(
            ...
            cache_miss_rate=1.0 - sum(r.performance.cache_hit_rate for r in results) / n,
            embedding_throughput=sum(r.performance.embedding_throughput for r in results) / n,
    ...
    def _build_recommendation(
        ...
        withdrawn: list[str] = []
        limitations = list(self.config.known_limitations) or [
        ...
        for result in comparison.results:
            if result.quality.candidate_recall < min_recall:
                withdrawn.append(
                    f"candidate_recall >= {min_recall} — {result.workflow_mode} achieved {result.quality.candidate_recall:.3f}"
                )
            if result.quality.source_quality_score < min_source_quality:
                withdrawn.append(
                    f"source_quality_score >= {min_source_quality} — {result.workflow_mode} achieved {result.quality.source_quality_score:.3f}"
                )
            if result.quality.coverage_completeness < min_coverage:
                withdrawn.append(
                    f"coverage_completeness >= {min_coverage} — {result.workflow_mode} achieved {result.quality.coverage_completeness:.3f}"
                )
            if result.quality.unsupported_claim_rate > max_unsupported:
                withdrawn.append(
                    f"unsupported_claim_rate <= {max_unsupported} — {result.workflow_mode} achieved {result.quality.unsupported_claim_rate:.3f}"
                )
            if result.quality.citation_accuracy < min_citation:
                withdrawn.append(
                    f"citation_accuracy >= {min_citation} — {result.workflow_mode} achieved {result.quality.citation_accuracy:.3f}"
                )
        baseline_perf = self._avg_performance(mode_results[first_mode])
        if baseline_perf:
            ...
            for mode, perf_results in mode_results.items():
                ...
                if avg_perf and baseline_perf.total_latency_ms > 0:
                    ...
                    if ratio > max_latency_ratio:
                        withdrawn.append(
                            f"latency_ratio <= {max_latency_ratio} — {mode} ratio {ratio:.2f} vs baseline"
                        )
                if avg_perf and baseline_perf.total_tokens > 0:
                    ...
                    if ratio > max_token_ratio:
                        withdrawn.append(
                            f"token_ratio <= {max_token_ratio} — {mode} ratio {ratio:.2f} vs baseline"
                        )
        p0_regressions = (
            ("deterministic integrity regression detected — at least one integrity check failed",)
...
def run_benchmark(
    ...
    return WorkflowBenchmarkRunner(loader, config).run()
