        ...
        try:
            if target.exists() and not target.is_dir():
                raise ExportWriteFailure(
                    f"target path exists as a file, not a directory: {target}"
                )
            ...
            raise ExportWriteFailure(
                f"atomic publication failed for {target}: {exc}"
            ) from exc
        finally:
...
def _map_event(
    row: dict[str, Any], run_catalog_id: str, invocation_ids: dict[str, str]
) -> dict[str, Any]:
    ...
    return {
        ...
        "invocation_id": invocation_ids.get(str(invocation_id))
        if invocation_id
        else None,
...
def _map_semantic_call(
    row: dict[str, Any], artifacts: list[dict[str, Any]], invocation_ids: dict[str, str]
) -> dict[str, Any]:
    invocation_id = row.get("invocation_id")
    return _sanitize(
        {
...
class CatalogExportService:
    ...
    def __init__(
        self, uow_factory: Callable, blob_store: ContentAddressedBlobStore | None = None
    ) -> None:
    ...
    def _load_projection(self, run_id: UUID) -> _Projection:
        with self.uow_factory() as uow:
            ...
            cur.execute(
                "SELECT * FROM research_invocations WHERE run_id=%s ORDER BY created_at,id",
                (str(run_id),),
            )
            invocations = _fetchall(cur)
            cur.execute(
                "SELECT * FROM research_events WHERE run_id=%s ORDER BY sequence_number,id",
                (str(run_id),),
            )
            events = _fetchall(cur)
            cur.execute(
                "SELECT * FROM research_claims WHERE run_id=%s ORDER BY created_at,id",
                (str(run_id),),
            )
            ...
            cur.execute(
                "SELECT * FROM audit_assessments WHERE run_id=%s ORDER BY created_at,id",
                (str(run_id),),
            )
            ...
            if assessments:
                ...
                stages = []
            cur.execute(
                "SELECT * FROM semantic_calls WHERE run_id=%s ORDER BY created_at,id",
                (str(run_id),),
            )
            calls = _fetchall(cur)
            if calls:
    ...
    def _snapshot_records(self, projection: _Projection) -> list[dict[str, Any]]:
        ...
        for row in projection.snapshot_rows:
            ...
            if not re.fullmatch(r"[0-9a-f]{64}", digest):
                raise ExportWriteFailure(
                    f"snapshot {row['id']} has invalid content SHA-256"
                )
            ...
            records.append(
                {
                    ...
                    "requested_url": _canonical_url(
                        row.get("requested_url") or canonical
                    ),
                    "title": row.get("title"),
    ...
    @staticmethod
    def _manifest(
        projection: _Projection, claims: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        for link in projection.evidence_rows:
            ...
            if claim_id not in known_claims:
                raise CatalogExportError(
                    f"evidence link references unknown claim {claim_id}"
                )
        ...
        for source in sources.values():
            ...
            source["relationships"].sort(
                key=lambda item: (item["claim_id"], item["passage_id"])
            )
            output.append(source)
        output.sort(
            key=lambda item: (item["canonical_url"], item.get("candidate_id") or "")
        )
    ...
    def _map_records(
        ...
        run_catalog_id = _catalog_id(
            RUN_PREFIX,
            run_row["id"],
            run_row.get("external_run_id") or run_row.get("external_id"),
        )
        invocation_ids = {
            str(row["id"]): _catalog_id(
                INVOCATION_PREFIX, row["id"], row.get("external_invocation_id")
            )
        ...
        for artifact in projection.semantic_artifact_rows:
            artifacts_by_call.setdefault(str(artifact["semantic_call_id"]), []).append(
                _sanitize(
                    {
        ...
        for row in projection.assessment_rows:
            ...
            target = (
                run_catalog_id
                if row.get("target_type") == "run"
                else invocation_ids.get(str(row.get("target_id")))
            )
            if target is None:
                raise CatalogExportError(
                    f"assessment {row['id']} references unknown target {row.get('target_id')}"
                )
            stage_values: dict[str, Any] = {}
            stage_errors: list[dict[str, Any]] = []
            call_ids = set(
                (row.get("audit_packet_manifest") or {}).get("semantic_call_ids", [])
            )
            ...
            unknown_calls = sorted(
                str(item) for item in call_ids if str(item) not in mapped_calls
            )
            if unknown_calls:
                raise CatalogExportError(
                ...
                _sanitize(
                    {
                        ...
                        "calls": [
                            mapped_calls[str(call_id)]
                            for call_id in sorted(call_ids, key=str)
                        ],
        ...
        for row in projection.invocation_rows:
            ...
            invocation_assessments = [
                ...
                if item["target_type"] == "invocation"
                and item["target_id"] == invocation_ids[str(row["id"])]
            ]
            invocations.append(
                _sanitize(
                    {
                        ...
                        "audit_status": invocation_assessments[-1]["status"]
                        if invocation_assessments
                        else "not_run",
                        "events": [],
        ...
        events = [
            _map_event(row, run_catalog_id, invocation_ids)
        ...
        for invocation in invocations:
            invocation["events"] = events_by_invocation.get(
        ...
        run = _sanitize(
            {
                ...
                "profile": {
                    "requested": run_row.get("execution_mode"),
                    "selected": run_row.get("execution_mode"),
                    "reason": "authoritative execution mode",
                },
                "started_at": _iso(run_row.get("started_at")),
                "updated_at": _iso(
                    run_row.get("updated_at") or run_row.get("started_at")
                ),
                "finished_at": _iso(
                    run_row.get("finished_at") or run_row.get("completed_at")
                ),
                "lifecycle": {
                    "state": "finished" if terminal else "running",
                    "revision": run_row.get("lifecycle_revision", 1),
                },
                "declared_outcome": outcome,
                "operational_status": operational_status,
                "data_completeness": "complete"
                if state == "completed" and claims and manifest["sources"]
                else "partial",
                "audit_status": run_assessments[-1]["status"]
                if run_assessments
                else "not_run",
                "assessment_summary": run_assessments[-1]["stages"].get("synthesis")
                if run_assessments
                else None,
                ...
                "annotations": [
                    event for event in events if event["event"] == "annotation"
                ],
                "final_answer": None,
                "assessment_refs": [
                    {
                ...
                "operational_summary": {
                    ...
                    "failed": sum(
                        item["execution"]["status"] == "failed" for item in invocations
                    ),
                    "snapshots": len(snapshots),
                    "claims": len(claims),
                    "coverage_revision": coverage.get(
                        "coverage_revision", run_row.get("current_coverage_revision", 0)
                    ),
                    "coverage": coverage.get("ledger", {}),
                },
                "evidence_revision": run_row.get(
                    "current_coverage_revision", run_row.get("lifecycle_revision", 1)
                ),
    ...
    def _source_hash(records: Iterable[Any]) -> str:
        return sha256(
            b"\x00".join(_canonical_bytes(item) for item in records)
        ).hexdigest()
    ...
    def _record_export(
        ...
        event_cursor = (
            projection.event_rows[-1]["id"] if projection.event_rows else None
        )
        with self.uow_factory() as uow:
    ...
    def _stage_tree(
        self, target_dir: Path, writer: Callable[[Path], None]
    ) -> list[Path]:
    ...
    @staticmethod
    def _invocation_snapshot_archive(
        invocation: dict[str, Any], snapshots: list[dict[str, Any]]
        ...
        for result in invocation.get("results", []):
            try:
                urls.add(
                    _canonical_url(result.get("canonical_url") or result.get("url"))
                )
        ...
        payload = {
            ...
            "topic": invocation.get("input", {}).get("topic")
            or invocation.get("input", {}).get("query")
            or "",
    ...
    def export_run(self, run_id: UUID, target_dir: Path | str) -> ExportRunResult:
        ...
        run, invocations, events, snapshots, claims, assessments, manifest = (
            self._map_records(projection)
        )
        ...
        try:
            ...
            def write(root: Path) -> None:
                ...
                _atomic_write_json(
                    root / "runs" / f"{run['research_run_id']}.json", run
                )
                for invocation in invocations:
                    _atomic_write_json(
                        root / "invocations" / f"{invocation['invocation_id']}.json",
                        invocation,
                    )
                _atomic_write_bytes(
                    root / "events.jsonl",
                    b"".join(_canonical_bytes(event) for event in events),
                )
    ...
    def _export_subset(
        ...
        writer_factory: Callable[
            [_Projection, tuple[Any, ...]], Callable[[Path], None]
        ],
    ) -> CatalogExportResult:
        run_id = UUID(str(run_id))
        target = Path(target_dir)
        ...
        export_id = (
            "ce_"
            + sha256(
                f"{source_hash}:{EXPORT_SCHEMA_VERSION}:{export_type}".encode()
            ).hexdigest()[:40]
        )
        attempt_key = f"catalog-v5:{export_type}-attempt:{uuid4()}"
        self._record_export(
            run_id, projection, target, export_type, source_hash, "pending", attempt_key
        )
        try:
    ...
    def export_invocation(
        self, invocation_id: UUID, run_id: UUID, target_dir: Path | str
    ) -> CatalogExportResult:
        ...
        invocation = next(
            (
                item
                for row, item in zip(projection.invocation_rows, records[1])
                if UUID(str(row["id"])) == invocation_id
            ),
            None,
        )
        if invocation is None:
            raise ExportTargetNotFound(
                f"invocation {invocation_id} not found for run {run_id}"
            )

        def factory(
            _projection: _Projection, _records: tuple[Any, ...]
        ) -> Callable[[Path], None]:
            return lambda root: _atomic_write_json(
                root / "invocations" / f"{invocation['invocation_id']}.json", invocation
            )

        result = self._export_subset(
            UUID(str(run_id)), target_dir, "catalog_v5_invocation", factory
        )
        return CatalogExportResult(
            result.export_id,
            result.source_state_sha256,
            result.export_schema_version,
            "invocation",
            str(invocation_id),
            result.target_dir,
            result.status,
    ...
    def export_events(
        ...
        return self._export_subset(
            UUID(str(run_id)),
            target_dir,
            "catalog_v5_events",
            lambda _p, r: (
                lambda root: _atomic_write_bytes(
                    root / "events.jsonl",
                    b"".join(_canonical_bytes(item) for item in r[2]),
                )
            ),
        )
    ...
    def export_snapshots(
        ...
        return self._export_subset(
            UUID(str(run_id)),
            target_dir,
            "catalog_v5_snapshots",
            lambda _p, r: lambda root: self._write_snapshots(root, r[3], r[1]),
        )
    ...
    def export_claims(
        ...
        return self._export_subset(
            ...
            target_dir,
            "catalog_v5_claims",
            lambda _p, r: lambda root: _atomic_write_json(root / "claims.json", r[4]),
        )
    ...
    def export_assessments(
        ...
        def factory(
            _projection: _Projection, records: tuple[Any, ...]
        ) -> Callable[[Path], None]:
            def write(root: Path) -> None:
        ...
        return self._export_subset(
            UUID(str(run_id)), target_dir, "catalog_v5_assessments", factory
        )
    ...
    def export_manifest(
        ...
        def factory(
            ...
            payload = {
                **records[6],
                "export_schema_version": EXPORT_SCHEMA_VERSION,
                "run_id": records[0]["research_run_id"],
                "source_state_sha256": source_hash,
            }
            return lambda root: _atomic_write_json(root / "manifest.json", payload)

        return self._export_subset(
            UUID(str(run_id)), target_dir, "catalog_v5_manifest", factory
        )
