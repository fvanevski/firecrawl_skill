...
POLICY = json.loads(
    (
        Path(__file__).resolve().parent.parent / "references" / "catalog-policy-v5.json"
    ).read_text()
)
...
def enabled():
    return os.environ.get("FIRECRAWL_CATALOG_DISABLED", "").lower() not in {
        "1",
        "true",
        "yes",
    }
...
def invocation_path(identifier):
    return catalog_root() / "invocations" / f"{identifier}.json"


def run_path(identifier):
    return catalog_root() / "runs" / f"{identifier}.json"


def assessment_path(identifier, assessment_id):
    return catalog_root() / "assessments" / identifier / f"{assessment_id}.json"
...
def read_path(path):
    ...
    if value and value.get("schema_version") not in SUPPORTED_SCHEMA_VERSIONS:
        raise SystemExit(
            f"ERROR: unsupported catalog schema {value.get('schema_version')}; migrate to schema 5"
        )
    return value


def read_record(identifier):
    return read_path(invocation_path(identifier))


def read_run(identifier):
    return read_path(run_path(identifier))
...
def stable_candidate_id(invocation_id, url, index):
    return (
        "fce_"
        + hashlib.sha256(
            f"{invocation_id}\0{canonical_url(url)}\0{index}".encode()
        ).hexdigest()[:24]
    )
...
def stable_excerpt_id(candidate_id, position, text):
    return (
        "fex_"
        + hashlib.sha256(f"{candidate_id}\0{position}\0{text}".encode()).hexdigest()[
            :24
        ]
    )
...
def collect_excerpts(entry, topic=""):
    ...
    for score, _, offset, block, matches in ranked:
        ...
        if budget <= 0 or len(output) >= int(
            SNAPSHOT_POLICY["max_excerpts_per_result"]
        ):
            break
            ...
            output.append(
                {
                    ...
                    "selection_reason": "objective_term_density",
            ...
            signals.append(
                {
                    ...
                    "parser_confidence": "high" if parse_time(entry[key]) else "low",
                }
            )
    evidence = " ".join(
        str(entry.get(key, "")) for key in ("url", "title", "snippet", "description")
    )
    evidence += " " + " ".join(
        item.get("text", "") for item in entry.get("excerpts", [])
    )
        ...
        signals.append(
            {
                ...
                "signal_type": "absolute_text",
                "parser_confidence": "medium",
            }
        )
    for match in re.finditer(
        r"\b(?:last updated|updated|published)\s+(?:on\s+)?([A-Z][a-z]+\s+\d{1,2},\s+20\d{2})",
        evidence,
        re.IGNORECASE,
    ):
        signals.append(
            {
                "value": match.group(1),
                "location": "visible_text",
                "signal_type": "labeled_text",
                "parser_confidence": "medium",
            }
        )
    for match in re.finditer(
        r"\b(\d{1,3})\s+(hour|day|week)s?\s+ago\b", evidence, re.IGNORECASE
    ):
        signals.append(
            {
                "value": match.group(0),
                "location": "visible_text",
                "signal_type": "relative_text",
                "anchor": acquired_at,
                "parser_confidence": "medium",
            }
        )
...
def source_hints(url):
    ...
    if host.endswith((".gov", ".mil", ".int")):
        hints.append("public-sector-domain")
    if host.endswith(".edu"):
        hints.append("academic-domain")
...
def constraint_domains(query):
    return [
        value.lower().strip('. )"')
        for value in re.findall(r"\bsite:([^\s]+)", query or "", re.IGNORECASE)
    ]
...
def host_matches(host, domain):
    return (
        ...
        else host == domain or host.endswith("." + domain)
    )
...
def scrub_result(
    ...
    if targeted:
        kept["targeted"] = True
    if kept.get("error"):
        kept["error_class"] = classify_error(kept["error"])
    kept["excerpts"] = collect_excerpts(entry, topic)
    kept["date_signals"] = collect_date_signals(
        {**entry, "excerpts": kept["excerpts"]}, acquired_at
    )
    ...
    kept["constraint"] = {
        ...
        "actual_host": host,
        "status": "compliant"
        if expected and any(host_matches(host, domain) for domain in expected)
        else "violated"
        if expected
        else "not_applicable",
    }
...
def requested_window(input_data):
    text = " ".join(
        ...
        for key in ("query", "topic", "objective", "run_objective")
    )
    tbs = str(input_data.get("tbs", "")).lower()
    match = re.search(r"qdr:([dwmy])", tbs)
    if match:
        return {"d": 1, "w": 7, "m": 31, "y": 366}[match.group(1)]
    match = re.search(
        r"\b(?:past|last|previous|latest)\s+(\d{1,3})\s+days?\b", text, re.IGNORECASE
    )
    return (
        int(match.group(1))
        if match
        else 1
        if re.search(r"\b(today|past day|last day)\b", text, re.IGNORECASE)
        else 7
        if re.search(r"\b(past week|last week)\b", text, re.IGNORECASE)
        else None
    )
...
def classify_error(error):
    ...
    if any(token in value for token in ("eai_again", "name or service", "connection")):
        return "network"
...
def normalize_results(meta, topic, acquired_at, invocation_id):
    ...
    if not raw and branches:
        raw = [
            ...
            for item in branch.get("metadata", {}).get("results", [])
        ]
    candidates = meta.get("candidates", []) or []
    branch_domains = {
        branch.get("query_index", pos): constraint_domains(
            branch.get("query_string") or branch.get("query") or ""
        )
        for pos, branch in enumerate(branches, 1)
    }
    output, by_url = [], {}
    direct = bool(meta.get("operation") == "scrape" or (raw and not candidates))
    for entry in candidates:
        domains = branch_domains.get(
            entry.get("branch") or entry.get("query_index"),
            constraint_domains(meta.get("query", "")),
        )
        item = scrub_result(
            entry,
            topic,
            acquired_at,
            requested_window({"topic": topic}),
            False,
            domains,
        )
    ...
    for index, item in enumerate(output):
        ...
        item["candidate_id"] = stable_candidate_id(
            invocation_id, item.get("canonical_url"), index
        )
        for position, excerpt in enumerate(item.get("excerpts", [])):
            excerpt["excerpt_id"] = stable_excerpt_id(
                item["candidate_id"], position, excerpt["text"]
            )
            excerpt["text_sha256"] = hashlib.sha256(
                excerpt["text"].encode()
            ).hexdigest()
    return output
...
def operational_metrics(meta, results, duration_ms=None):
    ok = [
        item
        for item in results
        if item.get("status") == "ok" or item.get("scrape_status") == "ok"
    ]
    errors = [
        item
        for item in results
        if item.get("status") == "error" or item.get("scrape_status") == "error"
    ]
    operation = meta.get("operation", "unknown")
    requested = (
        len(meta.get("urls", [])) or len(results)
        if operation == "scrape"
        else meta.get("candidate_count", len(results))
    )
    return {
        "operation": operation,
        ...
        "selected_count": sum(
            bool(item.get("selected") or item.get("targeted")) for item in results
        ),
        "successful_document_count": len(ok),
        "failed_document_count": len(errors),
        "unique_domain_count": len(
            {
                item.get("source_hints", {}).get("host")
                for item in results
                if item.get("source_hints", {}).get("host")
            }
        ),
        "constraint_violation_count": sum(
            item.get("constraint", {}).get("status") == "violated" for item in results
        ),
        "total_words": meta.get(
            "total_estimated_words",
            meta.get("estimated_total_words", meta.get("total_words", 0)),
        ),
...
def artifact_details(scratch_dir):
    if not scratch_dir or not Path(scratch_dir).is_dir():
        return []
    output = []
    for item in sorted(Path(scratch_dir).rglob("*")):
        if not item.is_file() or item.name.startswith(
            ("result_", "url_", "_search.json")
        ):
            continue
...
def persist_snapshot(invocation_id, topic, meta, results):
    snapshot = {
        ...
        "query_plan": sanitize(meta.get("query_plan", [])),
        "retry_query_plan": sanitize(meta.get("retry_query_plan", [])),
        "strategy": sanitize(meta.get("strategy", {})),
        "planner_provenance": sanitize(meta.get("planner_provenance", {})),
        "results": results,
        "truncation": {
            "truncated": False,
    ...
    if len(raw) > limit:
        ...
        for item in snapshot["results"]:
            trial = json.dumps(
                {**snapshot, "results": retained + [item]}, sort_keys=True
            ).encode()
            if len(trial) > limit:
                break
            retained.append(item)
        snapshot["results"] = retained
        snapshot["truncation"].update(
            truncated=True, reason="result_limit", retained_result_count=len(retained)
        )
        raw = json.dumps(snapshot, sort_keys=True).encode()
...
def _write_record(record, event_type, data=None):
    ...
    record["last_event_id"] = append_event(
        {
            "event": event_type,
...
def validate_run_id(run_id, require_open=False):
    if not re.fullmatch(r"fr_[0-9a-f]{32}", run_id or ""):
        raise SystemExit(
            "ERROR: research run ID must be fr_<32 lowercase hex characters>"
        )
    run = read_run(run_id)
    if not run:
        raise SystemExit(f"ERROR: no catalog research run for {run_id}")
    if require_open and run.get("lifecycle", {}).get("state") != "running":
        raise SystemExit(
            f"ERROR: research run {run_id} is finished; reopen it before attaching new work"
        )
    return run_id
    ...
    with catalog_lock(invocation_id):
        ...
        if meta_path and Path(meta_path).is_file():
            ...
            topic = (
                meta.get("topic")
                or meta.get("query")
                or record.get("input", {}).get("topic")
                or record.get("input", {}).get("query")
                or ""
            )
            results = normalize_results(
                meta, topic, record.get("started_at"), invocation_id
            )
            record.update(
                {
                    "scratch_dir": meta.get("scratch_dir") or meta.get("base_dir"),
                    "strategy": sanitize(meta.get("strategy", {})),
                    "planner_provenance": sanitize(meta.get("planner_provenance", {})),
                    "query_plan": sanitize(meta.get("query_plan", [])),
                    "retry_query_plan": sanitize(meta.get("retry_query_plan", [])),
                    ...
                    "operational_metrics": operational_metrics(
                        meta, results, record["execution"]["duration_ms"]
                    ),
                    "artifacts": artifact_details(
                        meta.get("scratch_dir") or meta.get("base_dir")
                    ),
                    "snapshot": persist_snapshot(invocation_id, topic, meta, results),
                }
            )
        record["operational_status"] = record["execution"]["status"]
        record["data_completeness"] = (
            "complete"
            if record.get("snapshot") or record.get("input", {}).get("dry_run")
            else "partial"
        )
        record["evidence_revision"] += 1
        _write_record(record, "invocation_finished")
    if record.get("research_run_id"):
        recompute_run(record["research_run_id"])
...
def detect_profile(objective, requested):
    ...
    if any(token in value for token in ("latest news", "past ", "breaking")):
        return "breaking_news", "current-news anchors"
    if any(
        token in value
        for token in (
            "legislation",
        ...
        return "legislative_legal", "legal anchors"
    if any(
        token in value for token in ("documentation", "api", "sdk", "cli", "framework")
    ):
        return "technical_docs", "technical anchors"
    return "general", "general objective"


def start_run(objective, profile="auto"):
    if not enabled():
        raise SystemExit("ERROR: catalog disabled")
    ...
    run = {
        ...
        "profile": {"requested": profile, "selected": selected, "reason": reason},
        "started_at": now(),
        "updated_at": now(),
    ...
    append_event(
        {
            "event": "run_started",
            "research_run_id": run_id,
            "data": {"objective": run["objective"], "profile": selected},
        }
    )
    print(run_id)
...
def load_source_manifest(path, used_urls):
    ...
    if path:
        try:
            ...
            raise SystemExit(f"ERROR: invalid source manifest: {exc}") from exc
    manifest.setdefault("claims", [])
...
def resolve_source_ledger(run, manifest):
    ...
    for record in filter(None, records):
        for result in record.get("results", []):
            choices.setdefault(result.get("canonical_url"), []).append((record, result))
    ...
    for source in manifest.get("sources", []):
        ...
        if unknown:
            raise SystemExit(
                f"ERROR: source references unknown claim IDs: {sorted(unknown)}"
            )
        ...
        if source.get("candidate_id"):
            matches = [
                item
                for item in matches
                if item[1].get("candidate_id") == source["candidate_id"]
            ]
        if source.get("invocation_id"):
            matches = [
                item
                for item in matches
                if item[0].get("invocation_id") == source["invocation_id"]
            ]
        if source.get("result_index") is not None:
            matches = [
                item
                for item in matches
                if item[1].get("result_index") == source["result_index"]
            ]
        if len(matches) > 1 and not (
            source.get("candidate_id")
            or (source.get("invocation_id") and source.get("result_index") is not None)
        ):
            raise SystemExit(
                f"ERROR: ambiguous source URL {source.get('url')}; specify candidate_id or invocation_id/result_index"
            )
        chosen = matches[0] if matches else None
        excerpts = source.get("excerpt_ids", [])
        if chosen:
            valid = {item.get("excerpt_id") for item in chosen[1].get("excerpts", [])}
            if set(excerpts) - valid:
                raise SystemExit(
                    f"ERROR: source references unknown excerpt IDs: {sorted(set(excerpts) - valid)}"
                )
        ledger.append(
            {
                "url": source.get("url"),
                "canonical_url": canonical_url(source.get("url", "")),
                "claim_ids": source.get("claim_ids", []),
                "roles": source.get("roles", []),
                "relationship": source.get("relationship", "supports"),
                "excerpt_ids": excerpts,
                "fidelity": source.get("fidelity", "manifest"),
                "resolution": "matched" if chosen else "unmatched",
                "invocation_id": chosen[0]["invocation_id"]
                if chosen
                else source.get("invocation_id"),
                "candidate_id": chosen[1].get("candidate_id")
                if chosen
                else source.get("candidate_id"),
                "result_index": chosen[1].get("result_index")
                if chosen
                else source.get("result_index"),
                "extraction_status": "ok"
                if chosen
                and (
                    chosen[1].get("status") == "ok"
                    or chosen[1].get("scrape_status") == "ok"
                )
                else "unknown",
                "date_signals": chosen[1].get("date_signals", []) if chosen else [],
                "source_hints": chosen[1].get("source_hints", {}) if chosen else {},
            }
        )
    return ledger
...
def recompute_run(run_or_id):
    ...
    records = [
        record
        for record in (read_record(item) for item in run.get("invocation_ids", []))
        if record
    ]
    metrics = [record.get("operational_metrics", {}) for record in records]
    used_invocations = {
        source.get("invocation_id")
        for source in run.get("used_sources", [])
        if source.get("invocation_id")
    }
    run["operational_summary"] = {
        "operations": len(records),
        "succeeded": sum(
            record.get("execution", {}).get("status") == "succeeded"
            for record in records
        ),
        "failed": sum(
            record.get("execution", {}).get("status") == "failed" for record in records
        ),
        "duration_ms": sum(item.get("duration_ms") or 0 for item in metrics),
        "candidates": sum(item.get("candidate_count") or 0 for item in metrics),
        "successful_documents": sum(
            item.get("successful_document_count") or 0 for item in metrics
        ),
        "failed_documents": sum(
            item.get("failed_document_count") or 0 for item in metrics
        ),
        "used_sources": len(run.get("used_sources", [])),
        "useful_operations": len(used_invocations),
    }
    ...
    run["data_completeness"] = (
        ...
        if run.get("claims")
        and bound_sources
        and all(source.get("excerpt_ids") for source in bound_sources)
        and run.get("final_answer")
        else "partial"
    )
    run["updated_at"] = now()
    run["record_revision"] += 1
    ...
    with catalog_lock(run_id):
        ...
        if run.get("lifecycle", {}).get("state") != "running":
            if (
                ...
                requested = _completion_inputs(used_urls, source_manifest, answer_file)
        ...
        if len(ids) != len(set(ids)):
            raise SystemExit("ERROR: claim IDs must be unique")
        ...
        if answer_file:
            ...
            run["final_answer"] = {
                "text": redact_text(text)[:120000],
                "sha256": hashlib.sha256(text.encode()).hexdigest(),
                "truncated": len(text) > 120000,
                "source_path": str(Path(answer_file)),
            }
        run["completion_inputs"] = _completion_inputs(
            used_urls, source_manifest, answer_file
        )
        run["declared_outcome"] = outcome
        run["lifecycle"] = {
            "state": "finished",
            "revision": run["lifecycle"]["revision"] + 1,
        }
        run["finished_at"] = now()
    ...
    append_event(
        {
            "event": "run_finished",
            "research_run_id": run_id,
            "data": {"outcome": outcome, "data_completeness": run["data_completeness"]},
        }
    )
    if os.environ.get("FIRECRAWL_AUDIT_AUTO_SEMANTIC", "1").lower() not in {
        "0",
        "false",
        "no",
    }:
...
def reopen_run(run_id, reason):
    with catalog_lock(run_id):
        ...
        run["declared_outcome"] = None
        run["claims"] = []
        run["used_sources"] = []
        run["final_answer"] = None
        run["completion_inputs"] = {}
        run["audit_status"] = "stale"
        run["evidence_revision"] += 1
        run.setdefault("annotations", []).append(
            {"type": "reopen", "reason": redact_text(reason), "at": now()}
        )
        run["record_revision"] += 1
        atomic_write(run_path(run_id), run)
    append_event(
        {
            "event": "run_reopened",
            "research_run_id": run_id,
            "data": {"reason": redact_text(reason)},
        }
    )
...
def annotate_run(run_id, event_type, reason, from_invocation=None, to_invocation=None):
    with catalog_lock(run_id):
        ...
        item = {
            ...
            "to_invocation": to_invocation,
        }
        run.setdefault("annotations", []).append(item)
        run["evidence_revision"] += 1
        run["audit_status"] = "stale"
        run["record_revision"] += 1
        atomic_write(run_path(run_id), run)
    append_event(
        {"event": f"run_{event_type}", "research_run_id": run_id, "data": item}
    )
...
def verify_target(identifier, persist=True):
    ...
    records = (
        [read_record(item) for item in run.get("invocation_ids", [])]
        if run
        else [read_record(identifier)]
    )
    checks = []
    for record in filter(None, records):
        for artifact in [
            *record.get("artifacts", []),
            *([record["snapshot"]] if record.get("snapshot") else []),
        ]:
    ...
    report = {
        ...
        "verified_at": now(),
        "total": len(checks),
        "available": sum(item["state"] == "available" for item in checks),
        "missing": sum(item["state"] == "missing" for item in checks),
        "hash_mismatch": sum(item["state"] == "hash_mismatch" for item in checks),
        "artifacts": checks,
    }
    if persist:
        append_event(
            {
                ...
                "research_run_id": identifier if run else None,
                "invocation_id": None if run else identifier,
                "data": {
                    key: report[key]
                    for key in ("total", "available", "missing", "hash_mismatch")
                },
            }
        )
    ...
    if stage == "rubric":
        return {
            ...
            "properties": {
                ...
                "corroboration_requirement": {"type": "string"},
                "high_stakes_claims": {"type": "array", "items": {"type": "string"}},
    ...
    if stage in {"acquisition", "evidence"}:
        return {
            ...
            "properties": {
                "stage_adequacy": {
                    "type": "string",
                    "enum": ["satisfactory", "partial", "weak", "indeterminate"],
                },
    ...
    return {
        ...
        "properties": {
            ...
            "declared_outcome_consistency": {
                "type": "string",
                "enum": ["consistent", "inconsistent", "indeterminate"],
            },
            "confidence": {"type": "number", "minimum": 0, "maximum": 1},
    ...
    if path.is_file():
        for line in path.read_text(encoding="utf-8").splitlines():
            try:
                ...
                if item.get("research_run_id") == run_id and item.get("event") not in {
                    "assessment_finished",
...
def build_audit_packet(identifier):
    ...
    if not target:
        raise SystemExit(f"ERROR: no catalog target for {identifier}")
    records = (
        [read_record(item) for item in run.get("invocation_ids", [])]
        if run
        else [target]
    ...
    used_ids = {
        source.get("candidate_id") for source in (run or {}).get("used_sources", [])
    }
    for record in records:
        operations.append(
            {
...
def _known_refs(packet):
    ...
    for item in packet.get("used_source_dossiers", []):
        refs.update(excerpt.get("excerpt_id") for excerpt in item.get("excerpts", []))
...
def validate_stage_output(stage, value, packet):
    ...
    findings = (
        value.get("findings", [])
        + value.get("dimension_judgments", [])
        + value.get("priority_recommendations", [])
    )
...
def normalize_evidence_refs(value, packet):
    ...
    def resolve(ref):
        ...
        patterns = (
            ...
            (
                r"used_source_dossiers\[(\d+)\]\.excerpts\[(\d+)\]",
        ...
        for pattern, collection, key in patterns:
            ...
            try:
                ...
                if (
                    collection == "used_source_dossiers"
...
def _candidate_chunks(packet, target_tokens):
    ...
    for card in cards:
        ...
        if current and estimate_tokens(trial) > target_tokens:
            chunks.append({**base, "candidate_cards": current})
            current = [card]
        else:
            current.append(card)
    ...
    for index, chunk in enumerate(chunks, 1):
        chunk["context_manifest"] = {
            **packet["context_manifest"],
            "chunk": index,
            "chunk_count": len(chunks),
            "candidate_count_in_chunk": len(chunk["candidate_cards"]),
            "omissions": ["candidate ledger split across acquisition chunks"]
            if len(chunks) > 1
            else [],
        }
    return chunks
...
def _run_stage(stage, provider, model, payload, max_output_tokens):
    ...
    return call_structured(
        ...
        timeout=int(AUDIT_POLICY["call_timeout_seconds"]),
        prompt_version=PROMPT_TEMPLATE_VERSION + ":" + stage,
    )


def _run_stage_with_fallback(
    stage, provider, model, payload, fallback_provider, fallback_model
):
    primary = _run_stage(
        stage, provider, model, payload, int(AUDIT_POLICY["max_output_tokens"])
    )
    ...
    if (
        ...
        raise SystemExit(
            "ERROR: commercial fallback requires local primary plus --commercial-fallback openai|gemini and --fallback-model"
        )
    fallback = _run_stage(
        stage,
        fallback_provider,
        fallback_model,
        payload,
        int(AUDIT_POLICY["max_output_tokens"]),
    )
...
def audit_target(
    ...
    max_calls = max_calls or int(AUDIT_POLICY["max_calls"])
    target_tokens = max_input_tokens or int(AUDIT_POLICY["target_input_tokens"])
    ...
    stage_payloads = [
        ("rubric", {"objective": packet["objective"], "profile": packet["profile"]})
    ]
    ...
    for stage, payload in stage_payloads:
        if stage not in requested or len(calls) >= max_calls:
            continue
        result, primary_failure = _run_stage_with_fallback(
            stage, provider, model, payload, fallback_provider, fallback_model
        )
        if primary_failure:
            calls.append(
                {
                    ...
                    "provenance": primary_failure.provenance,
                    "attempts": primary_failure.attempts,
        ...
        if result.value:
            ...
            if valid:
                outputs.setdefault(stage, []).append(result.value)
            else:
                stage_errors.append(
                    {
                        "stage": stage,
            ...
            stage_errors.append({"stage": stage, "error": result.error})
        if time.monotonic() - started > int(AUDIT_POLICY["max_total_seconds"]):
            stage_errors.append(
                {"stage": stage, "error": "automatic audit total-time budget exhausted"}
            )
            break
    if "synthesis" in requested and len(calls) < max_calls:
        synthesis_payload = {
            "packet_identity": {
                ...
                "declared_outcome": packet["declared_outcome"],
                "operational_summary": packet["operational_summary"],
            },
            "rubric": outputs.get("rubric", []),
            "acquisition_assessments": outputs.get("acquisition", []),
            "evidence_assessments": outputs.get("evidence", []),
        ...
        if result.value:
            ...
            if valid:
                outputs["synthesis"] = result.value
            else:
                stage_errors.append(
                    {
                        "stage": "synthesis",
            ...
            stage_errors.append({"stage": "synthesis", "error": result.error})
    status = (
        "completed"
        if outputs.get("synthesis") and not stage_errors
        else "partial"
        if outputs
        else "failed"
    )
    ...
    assessment = {
        ...
        "created_at": now(),
        "status": status,
        "provider_policy": "local-first-commercial-opt-in",
        "evidence_scope": "audit_packet_with_bounded_excerpts",
    ...
    path = (
        ...
        else invocation_path(identifier)
    )
    with catalog_lock(identifier):
        ...
        target.setdefault("assessment_refs", []).append(
            {
                "assessment_id": assessment_id,
                "status": status,
                "provider": provider,
        ...
        if outputs.get("synthesis"):
            target["assessment_summary"] = outputs["synthesis"]
    ...
    append_event(
        {
            ...
            if identifier.startswith(RUN_PREFIX)
            else target.get("research_run_id"),
            "invocation_id": None if identifier.startswith(RUN_PREFIX) else identifier,
            "data": {
                "assessment_id": assessment_id,
                "provider": provider,
                "status": status,
...
def assessment_refs_with_state(identifier, target=None):
    ...
    current = hashlib.sha256(
        json.dumps(build_audit_packet(identifier), sort_keys=True).encode()
    ).hexdigest()
    output = []
    for ref in target.get("assessment_refs", []):
        ...
        item["freshness"] = (
            ...
            if item.get("target_hash") == current
            and item.get("evaluator_version") == EVALUATOR_VERSION
            else "stale"
        )
        output.append(item)
    return output
...
def compact_run(run):
    return {
        ...
        "assessment_refs": assessment_refs_with_state(run["research_run_id"], run),
        "invocations": [
            summarize(record)
            for record in (read_record(item) for item in run.get("invocation_ids", []))
            if record
...
def summarize(record):
    return {
        ...
        "topic": record.get("input", {}).get("query")
        or record.get("input", {}).get("topic")
        or "",
        "metrics": record.get("operational_metrics", {}),
    }
...
def show_target(identifier, raw=False, verbose=False):
    ...
    print(
        json.dumps(
            target
            if raw or verbose
            else compact_run(target)
            if identifier.startswith(RUN_PREFIX)
            else summarize(target),
            indent=2,
            sort_keys=True,
        )
    )
...
def compare_runs(run_ids):
    ...
    for identifier in run_ids:
        ...
        if not run:
            raise SystemExit(f"ERROR: no catalog research run for {identifier}")
        comparison.append(
            {
                "research_run_id": identifier,
                "schema_version": 5,
                "objective": run.get("objective"),
                "profile": run.get("profile", {}).get("selected"),
                "operational_status": run.get("operational_status"),
                "data_completeness": run.get("data_completeness"),
                "audit_status": run.get("audit_status"),
                "assessment_summary": run.get("assessment_summary"),
                "operational_summary": run.get("operational_summary"),
                "assessment_count": len(run.get("assessment_refs", [])),
                "evaluator_version": EVALUATOR_VERSION,
                "policy_version": POLICY_VERSION,
            }
        )
...
def aggregate_catalog():
    ...
    for run in filter(None, runs):
        ...
        for finding in summary.get("dimension_judgments", []) + summary.get(
            "priority_recommendations", []
        ):
            code = finding.get("code", "UNSPECIFIED")
            codes.setdefault(code, {"count": 0, "runs": []})
            codes[code]["count"] += 1
            codes[code]["runs"].append(run["research_run_id"])
    print(
        json.dumps(
            {
                "schema_version": 5,
                "run_count": len([item for item in runs if item]),
                "recurring_findings": dict(
                    sorted(codes.items(), key=lambda item: -item[1]["count"])
                ),
            },
            indent=2,
            sort_keys=True,
...
def list_catalog():
    root = catalog_root()
    for run in sorted(
        filter(None, (read_path(path) for path in (root / "runs").glob("fr_*.json"))),
        key=lambda item: item.get("started_at", ""),
        reverse=True,
    ):
        print(
            f"{run['research_run_id']} [{run.get('lifecycle', {}).get('state')}/{run.get('audit_status')}] profile={run.get('profile', {}).get('selected')} operations={len(run.get('invocation_ids', []))} objective={run.get('objective', '')}"
        )
    for record in sorted(
        filter(
            None, (read_path(path) for path in (root / "invocations").glob("fc_*.json"))
        ),
        key=lambda item: item.get("started_at", ""),
        reverse=True,
    ):
        metric = record.get("operational_metrics", {})
        label = (
            f"candidates={metric.get('candidate_count', 0)}"
            if record.get("operation") != "scrape"
            else f"documents={metric.get('successful_document_count', 0)}/{metric.get('requested_document_count', 0)}"
        )
        print(
            f"{record['invocation_id']} [{record.get('operational_status')}/{record.get('audit_status')}] {record.get('operation')} run={record.get('research_run_id') or '-'} {label}"
        )
...
def recompute_target(identifier):
    if identifier.startswith(RUN_PREFIX):
        print(
            json.dumps(compact_run(recompute_run(identifier)), indent=2, sort_keys=True)
        )
    else:
        record = read_record(identifier)
        if not record:
            raise SystemExit(f"ERROR: no catalog invocation for {identifier}")
...
def migrate_catalog(apply=False):
    root = catalog_root()
    marker = (
        read_path(root / "catalog.json") if (root / "catalog.json").is_file() else None
    )
    if marker and marker.get("schema_version") == 5:
        ...
        result = {
            "action": "dry_run",
            "from_schema": marker.get("schema_version") if marker else "unknown",
            "to_schema": 5,
            "would_discard_entire_catalog": root.exists(),
        ...
        atomic_write(
            root / "catalog.json",
            {
                "schema_version": 5,
                "initialized_at": now(),
                "history_policy": "discard_on_schema_change",
            },
        )
        append_event(
            {"event": "catalog_schema_initialized", "data": {"schema_version": 5}}
        )
...
def purge_catalog(force=False, before=None, run_id=None, keep_last=None, orphans=False):
    root = catalog_root()
    if not root.exists():
        print(json.dumps({"action": "no_change", "targets": []}))
        return
    if not any((before, run_id, keep_last is not None, orphans)):
        ...
        print(
            json.dumps(
                {"action": "purged" if force else "dry_run", "targets": targets},
                indent=2,
            )
        )
        return
    ...
    if keep_last is not None:
        remove.update(item["research_run_id"] for item in runs[keep_last:])
    if before:
        ...
        remove.update(
            ...
            and parse_time(item["started_at"]) < cutoff
        )
    invocation_ids = {
        item
        for run in runs
        if run.get("research_run_id") in remove
        for item in run.get("invocation_ids", [])
    }
    if orphans:
        linked = {item for run in runs for item in run.get("invocation_ids", [])}
        invocation_ids.update(
            path.stem
            for path in (root / "invocations").glob("fc_*.json")
            if path.stem not in linked
        )
    targets = [
        *(root / "runs" / f"{item}.json" for item in remove),
        *(root / "invocations" / f"{item}.json" for item in invocation_ids),
        *(root / "snapshots" / f"{item}.json.gz" for item in invocation_ids),
        *(root / "assessments" / item for item in remove | invocation_ids),
    ]
    if force:
        for path in targets:
            if path.is_dir():
        ...
        if events.is_file():
            kept = [
                ...
                if not any(identifier in line for identifier in remove | invocation_ids)
            ]
            events.write_text("\n".join(kept) + ("\n" if kept else ""))
    print(
        json.dumps(
            {
                "action": "purged" if force else "dry_run",
...
def main():
    ...
    item.add_argument("--invocation-id", required=True)
    item.add_argument("--operation", required=True)
    item.add_argument("--input-json", default="{}")
    item.add_argument("--research-run-id")
    item.set_defaults(
        func=lambda a: begin(
            a.invocation_id, a.operation, load_json(a.input_json, {}), a.research_run_id
        )
    )
    item = sub.add_parser("event")
    item.add_argument("--invocation-id", required=True)
    item.add_argument("--event-type", required=True)
    item.add_argument("--data-json", default="{}")
    item.set_defaults(
        func=lambda a: add_event(
            a.invocation_id, a.event_type, load_json(a.data_json, {})
        )
    )
    item = sub.add_parser("finish")
    item.add_argument("--invocation-id", required=True)
    item.add_argument("--status", choices=("completed", "failed"), required=True)
    item.add_argument("--meta")
    item.add_argument("--error", default="")
    item.add_argument("--exit-code", type=int)
    item.set_defaults(
        func=lambda a: complete(a.invocation_id, a.status, a.meta, a.error, a.exit_code)
    )
    item = sub.add_parser("run-start")
    ...
    item.set_defaults(func=lambda a: start_run(a.objective, a.profile))
    item = sub.add_parser("run-finish")
    item.add_argument("run_id")
    item.add_argument(
        "--outcome", choices=("satisfied", "partial", "failed"), required=True
    )
    item.add_argument("--used-url", action="append", default=[])
    item.add_argument("--source-manifest")
    item.add_argument("--answer-file")
    item.set_defaults(
        func=lambda a: finish_run(
            a.run_id, a.outcome, a.used_url, a.source_manifest, a.answer_file
        )
    )
    item = sub.add_parser("run-reopen")
    item.add_argument("run_id")
    item.add_argument("--reason", required=True)
    item.set_defaults(func=lambda a: reopen_run(a.run_id, a.reason))
    item = sub.add_parser("run-annotate")
    item.add_argument("run_id")
    item.add_argument("--type", choices=("pivot", "retry", "decision"), required=True)
    item.add_argument("--reason", required=True)
    item.add_argument("--from-invocation")
    item.add_argument("--to-invocation")
    item.set_defaults(
        func=lambda a: annotate_run(
            a.run_id, a.type, a.reason, a.from_invocation, a.to_invocation
        )
    )
    item = sub.add_parser("verify")
    item.add_argument("id")
    item.set_defaults(func=lambda a: verify_target(a.id))
    item = sub.add_parser("audit")
    item.add_argument("id")
    item.add_argument("--llm", choices=("local", "openai", "gemini"), default="local")
    item.add_argument("--model")
    item.add_argument("--force", action="store_true")
    item.add_argument("--stages")
    item.add_argument("--max-calls", type=int)
    item.add_argument("--max-input-tokens", type=int)
    item.add_argument("--commercial-fallback", choices=("openai", "gemini"))
    item.add_argument("--fallback-model")
    item.add_argument("--auto-semantic", action="store_true")
    item.set_defaults(
        func=lambda a: audit_target(
            a.id,
            a.llm,
            a.model,
            a.force,
            False,
            a.commercial_fallback,
            a.fallback_model,
            a.stages.split(",") if a.stages else None,
            a.max_calls,
            a.max_input_tokens,
        )
    )
    item = sub.add_parser("audit-status")
    item.add_argument("id")
    item.set_defaults(func=lambda a: show_target(a.id))
    item = sub.add_parser("run-show")
    item.add_argument("run_id")
    item.add_argument("--json", action="store_true")
    item.add_argument("--verbose", action="store_true")
    item.set_defaults(func=lambda a: show_target(a.run_id, a.json, a.verbose))
    item = sub.add_parser("show")
    item.add_argument("id")
    item.add_argument("--json", action="store_true")
    item.add_argument("--verbose", action="store_true")
    item.set_defaults(func=lambda a: show_target(a.id, a.json, a.verbose))
    item = sub.add_parser("compare")
    item.add_argument("run_ids", nargs="+")
    item.set_defaults(func=lambda a: compare_runs(a.run_ids))
    item = sub.add_parser("aggregate")
    item.set_defaults(func=lambda a: aggregate_catalog())
    item = sub.add_parser("recompute")
    item.add_argument("id")
    item.set_defaults(func=lambda a: recompute_target(a.id))
    item = sub.add_parser("migrate")
    item.add_argument("--from", dest="from_schema", type=int, choices=(4,), default=4)
    item.add_argument("--to", dest="to_schema", type=int, choices=(5,), default=5)
    item.add_argument("--apply", action="store_true")
    item.set_defaults(func=lambda a: migrate_catalog(a.apply))
    item = sub.add_parser("purge")
    item.add_argument("--force", action="store_true")
    item.add_argument("--before")
    item.add_argument("--run-id")
    item.add_argument("--keep-last", type=int)
    item.add_argument("--orphans", action="store_true")
    item.set_defaults(
        func=lambda a: purge_catalog(
            a.force, a.before, a.run_id, a.keep_last, a.orphans
        )
    )
    item = sub.add_parser("list")
    item.set_defaults(func=lambda a: list_catalog())
    item = sub.add_parser("export-to-fs")
    item.add_argument("--invocation-id", required=True)
    item.add_argument("--run-id")
    item.set_defaults(
        func=lambda a: print(
            export_invocation_to_filesystem(a.invocation_id, a.run_id) or "", end=""
        )
    )
    item = sub.add_parser("export-events-to-fs")
    item.add_argument("--run-id", required=True)
    item.set_defaults(
        func=lambda a: print(export_run_events_to_filesystem(a.run_id) or "", end="")
    )
