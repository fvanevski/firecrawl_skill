...
class TestQualityAcrossContentTypes:
    ...
    def test_concise_notice_acceptable(self, fixture_concise_notice):
        """Concise notice with a heading is unambiguously acceptable.

        B-5 fix: the fixture now contains a title heading so that
        title_present=True and extraction_method_confidence is elevated.
        Short valid sources must evaluate to "acceptable", not merely
        ...
        assert metrics.title_present, (
            "Fixture must have a title heading so the quality evaluator "
            "can determine it is a valid short document."
        )
        ...
        # A concise notice with a title heading must be acceptable —
        # not rejected purely for length.
        assert disposition == "acceptable", (
            f"Short valid source must be 'acceptable', got {disposition!r}. "
            f"visible_text_length={metrics.visible_text_length}, "
            f"title_present={metrics.title_present}, "
            f"extraction_method_confidence={metrics.extraction_method_confidence:.4f}"
        )

    def test_anti_bot_poor(self, fixture_anti_bot):
        """Anti-bot content is automatically mapped to "poor" by the quality evaluator.

        B-6 fix: verify that the quality evaluator + QualityService together
        produce disposition="poor" — not just that anti_bot_markers > 0.
        ...
        # The disposition must be "poor" — not just "not acceptable".
        service = QualityService(
            MagicMock(), config=QualityConfig(anti_bot_hard_fail=True)
        )
        disposition = service.map_disposition(metrics)
        assert disposition == "poor", (
            f"Anti-bot content must be 'poor', got {disposition!r}. "
            f"anti_bot_markers={metrics.anti_bot_markers}, "
            f"visible_text_length={metrics.visible_text_length}"
        )
...
class TestChunkingInvariants:
    """Unit tests for chunking hard invariants — runs without a database.

    B-4 fix: test_oversized_block_is_split was previously inside the
    @_integration() class TestE2EExtractionPipeline and was silently skipped
    in environments without a database.  The token-boundary invariant is pure
    computation and must run as a unit test.
    """

    def test_oversized_block_is_split(self):
        """Oversized block is split into multiple chunks, all within the token limit.

        Verifies the hard token-limit invariant: no chunk produced by
        ...
        assert len(chunks) >= 2, (
            f"Expected >= 2 chunks for oversized block, got {len(chunks)}"
        )
        # No chunk may exceed max_tokens — this is a hard invariant.
        for chunk in chunks:
            assert chunk.token_count <= 100, (
                f"Chunk {chunk.ordinal} has {chunk.token_count} tokens, "
                f"exceeds max_tokens=100"
            )
...
class TestFaultInjection:
    ...
    def test_database_commit_failure_preserves_attempt(
        ...
        B-1 fix: the previous implementation used _FailingUnitOfWork whose
        __exit__ was a no-op — commit() was never called, so no exception
        fired, and the assertion queried through the still-patched factory.

        The fix wraps a real PostgresUnitOfWork in a shim that forces
        ROLLBACK on __exit__ regardless of success or failure, then verifies
        with a fresh, unpatched connection that no row was committed.
        ...
        class _RollbackUnitOfWork:
            ...
            def __exit__(self_inner, exc_type, exc, tb):
                # Force ROLLBACK regardless of whether the body succeeded.
                try:
                    self_inner._real_uow.connection.rollback()
                finally:
                    self_inner._real_uow.connection.close()
                # Suppress nothing — re-raise any exception from the body.
                return False

        def _rollback_factory():
            return _RollbackUnitOfWork()

        # Swap in the rollback-forcing factory for one create_attempt call.
        original_uow_factory = e2e_extraction_service.uow_factory
        e2e_extraction_service.uow_factory = _rollback_factory
        try:
            # create_attempt may succeed or raise depending on timing;
            # either way the transaction must not be committed.
            try:
            ...
            e2e_extraction_service.uow_factory = original_uow_factory

        # Verify with a fresh, unpatched connection that no row was committed.
        attempts = e2e_extraction_service.list_attempts(
            sample_candidate, run_id=sample_run
        )
        assert len(attempts) == 0, (
            f"Rolled-back attempt must not be visible, found {len(attempts)} row(s). "
            "The UoW atomicity guarantee has been violated."
        )
    ...
    def test_no_ingestion_without_selection(
        ...
        """No corpus ingestion without a selected attempt leaves no provenance linkage.

        B-2 fix: the previous test ended with a comment and never called
        CorpusService.ingest.  This fix calls ingest directly and asserts
        that the resulting document has no extraction_attempt_id linkage —
        demonstrating that ingestion without a prior selection produces a
        ...
        # Confirm that no attempt has been selected.
        selected = e2e_extraction_service.get_selected_attempt(sample_candidate)
        assert selected is None

        # Attempt corpus ingestion WITHOUT a selected attempt.
        # CorpusService.ingest does not itself enforce the selection gate —
        # that is the orchestrator's responsibility.  What we verify here is
        # the observable consequence: the document created by ingest has a
        # null extraction_attempt_id, meaning the provenance gap is detectable.
        config = _make_config(tmp_path)
        corpus_service = CorpusService(
            config=config,
            uow_factory=e2e_extraction_service.uow_factory,
        ...
        request = IngestRequest(
            ...
            content=content,
            mime_type="text/markdown",
            # Deliberately omit extraction_attempt_id — simulating ingestion
            # that bypassed the selection gate.
        )
        ...
        with e2e_extraction_service.uow_factory() as uow:
            with uow.connection.cursor() as cur:
                cur.execute(
                    ...
                    (str(ingest_result.document_id),),
                )
                row = cur.fetchone()
                assert row is not None, "Document must exist in the database"
                extraction_attempt_id = row[0]
                assert extraction_attempt_id is None, (
                    "Document ingested without a selected attempt must have "
                    f"extraction_attempt_id=NULL, got {extraction_attempt_id!r}. "
                    "This makes the provenance gap detectable and auditable."
                )
...
class TestRedriveReindexFlow:
    ...
    def test_rederive_preserves_source_snapshot(
        ...
        """Re-derive with a new parser version must not overwrite the source snapshot.

        B-7 fix: verifies that calling rederive() with a different parser
        version creates a new document derivation without deleting or overwriting:
        (a) the original asset_snapshot row, and
        (b) the original document row.

        Both the original and new document rows must reference the same
        ...
        request = IngestRequest(
            requested_url="https://test.example.com/rederive-snapshot-test",
            content=fixture_concise_notice,
            mime_type="text/markdown",
            extraction_attempt_id=attempt_id,
        )
        ingest_result = corpus_service.ingest(request)
        original_snapshot_id = ingest_result.snapshot_id
        original_document_id = ingest_result.document_id

        # ---- Rederive with parser markdown-v2 ----
        derivation_service = DerivationService(
            uow_factory=e2e_extraction_service.uow_factory,
            corpus_service=corpus_service,
            blob_root=tmp_path / "blobs",
        ...
        with e2e_extraction_service.uow_factory() as uow:
            with uow.connection.cursor() as cur:
                ...
                cur.execute(
                    "SELECT id FROM asset_snapshots WHERE id = %s",
                    (str(original_snapshot_id),),
                )
                assert cur.fetchone() is not None, (
                    "Original source snapshot must not be deleted or overwritten "
                    "by rederive — raw bytes are content-addressed and immutable."
                )

                # (b) The original document row must still exist.
                cur.execute(
                    "SELECT id FROM documents WHERE id = %s",
                    (str(original_document_id),),
                )
                assert cur.fetchone() is not None, (
                    "Original document row must not be deleted or overwritten "
                    "by rederive — derivation is always append-only."
                )
