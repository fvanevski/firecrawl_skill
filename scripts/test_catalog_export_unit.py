...
def test_checked_in_golden_layout_and_real_readers(tmp_path, monkeypatch):
    ...
    assert (
        catalog_v5.read_record(invocation_path.stem)["invocation_id"]
        == invocation_path.stem
    )
    assert (
        catalog_v5.read_path(
            catalog_v5.assessment_path(
                golden["run"]["research_run_id"], assessment_path.stem
            )
        )["calls"]
        == []
    )
    assert catalog_v5._events_for_run(golden["run"]["research_run_id"]) == [
        golden["event"]
    ]
    assert (
        catalog_v5.build_audit_packet(golden["run"]["research_run_id"])["target_id"]
        == golden["run"]["research_run_id"]
    )
